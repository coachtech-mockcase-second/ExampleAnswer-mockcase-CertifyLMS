 ...
 *
 * `Admin\Announcement\StoreAction` から、解決済の対象 User Collection を巡回しつつ呼ばれる。
 * 受信者の `status !== InProgress` は配信スキップして dispatched_count に算入しない (呼出側で件数集計)。
 *
 * @return bool 配信したら true (呼出側で件数カウントに利用)
 */
final class NotifyAnnouncementAction
{
    public function __invoke(Announcement $announcement, User $recipient): bool
    {

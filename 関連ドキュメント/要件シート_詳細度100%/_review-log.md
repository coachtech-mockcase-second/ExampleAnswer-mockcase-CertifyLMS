...
### 解決マーク / 解除を冪等トグルに変更(2026-05-28、模範解答 PJ 修正)

ユーザー指摘「重複操作 409 は本当に適切か」を受けて再評価。resolve / unresolve は 2 状態(open ⇄ resolved)のトグルで、UI は現在状態に応じて片方のボタンしか出さないため、409 が出るのは二重送信 / 古いタブ / 直 POST / 並行リクエストのみ。HTTP 409 は本来「現在状態との衝突」を指すが "既に目的状態" は一致であり意味的にも噛み合わず、GitHub の issue 再クローズ等 実在の同種トグルも基本冪等。多状態マスタ(Plan / MeetingPack)の遷移ガード(不正遷移を 409 で拒否)とは別物と整理し、2 状態トグルには冪等性を採用。

**修正(模範解答 PJ)**:

- `ResolveAction` / `UnresolveAction` を冪等 no-op 化(既に目的状態なら early return、resolve は `resolved_at` を上書きしない)。`QaThreadAlreadyResolvedException` / `QaThreadNotResolvedException` は不要になり **削除**(`QaThreadHasRepliesException` は削除ガードとして存続)。
- テスト更新: `ResolveTest`(double resolve / unresolve open の 409 → リダイレクト + flash success + 状態・解決日時不変)/ `ResolveActionTest`・`UnresolveActionTest`(例外 expect → no-op + `resolved_at` 保持を検証)。
- 検証: qa-board 関連 **83 テスト全 pass**(SQLite in-memory、186 assertions)。
- チケット S-B-01 の AC 4 / インターフェース / 異常系 / コンポーネント例外 / 設計判断 / Q&A を冪等トグル表現に整合。
...
- `CertificateController::download` は `auth` のみ(`active-learning` 非適用)+ `authorize('download')` → `DownloadAction`。`Certificate\IssueAction` は修了状態ガード → `DB::transaction` 内で `lockForUpdate` 二重発行検出 → `CertificatePdfService::generate`(失敗時 Storage 保険削除 + `CertificateGenerationFailedException`)。`CertificatePolicy::download` は admin 全件 / student 本人 / coach 担当資格(`loadMissing('certification.coaches')`)
...
- **S-A-04**: AC 22 → 9 件に圧縮(発行ガード×2 / 発行成功+PDF 実体 / 失敗ロールバック / PDF 内容 / DL 認可 / DL 応答 / PDF 不在 / 学習中以外 / テスト)。`概要` 削除、トップレベルの `Seeder 設計` 節を実装方針 > データモデル > 初期データ Seeder に統合。12 個あった `アーキテクチャ判断` を本チケット固有のトレードオフのみの `設計判断` に整理、クラス列挙はコンポーネント節に集約
...
| `AnnouncementNotification` の `toDatabase()` で `link_route='notifications.show'`(通知詳細ページ) | ✅ 適切(再検討済 2026-05-31) | 当初は `notifications.index`(通知一覧へ戻るだけで実質遷移先なし)だったが、お知らせは遷移先となる業務画面を持たず本文全文が画面で読めない問題があったため、通知詳細ページ(`NotificationController::show`、認可は受信者本人か = `NotificationPolicy::view` のみ、開封時に既読化)へ遷移し本文全文を表示する方式に変更。お知らせをエンティティとして扱う独立画面(配信対象判定の認可が必要)は設けず、通知の所有チェックで完結させる。`S-B-04` の `data.link_route` 共通キー仕様準拠 |
...
| S-A-04 | certification-management | ✅ 適切 | Service / Action(`CertificatePdfService` / `Certificate\{Issue,Download}Action`)| `certificates` table + 修了証 INSERT は **提供 PJ 既存**(`ReceiveCertificateAction` から呼ばれる)で、本チケットは **PDF 生成 + Storage 保存 + DL endpoint** を追加する構造。PDF 生成失敗時の DB ROLLBACK + Storage 保険削除 + Service Mockery 不採用(`final` 外し)が `backend-exceptions.md` / `backend-services.md` 規約準拠。Step 4 = PDF 生成部分 + DL Action + Controller + Policy + Blade + Service 1 件 + Exception 2 件 + composer 追加 を引き算で剥がし、提供 PJ では Certificate INSERT のみで `pdf_path` がパス予約のみの状態に巻き戻す |
...
### T-A-04 / horizontal-04(⚠️→✅ 模範解答 PJ 未実装ギャップ検出 → ユーザー判断(A)で対応完了)
...
- **判定**: ⚠️→**✅ 対応完了**(2026-05-29、ユーザー判断(A)= 模範解答 PJ 側でギャップを埋める)。commit `007063b` でギャップ #1〜#3 を解消済を確認 — `tests/Unit/Services/Google/GoogleOAuthServiceTest.php`(5 ケース: 認可 URL / code 交換成功・エラー / revoke true・false)+ `GoogleCalendarServiceTest.php`(11 ケース: freebusy 正常・空・期限切れリフレッシュ成功/失敗・API 例外 / insertEvent 成功・失敗・refresh 失敗 / deleteEvent 410・他例外・正常)が新設、両クラスに `#[Group('external')]` + `setUp()` で `Http::preventStrayRequests()`。`./vendor/bin/sail bin phpunit tests/Unit/Services/Google/` で **16/16 pass(24 assertions)** を当方でも再確認(`PDO::MYSQL_ATTR_SSL_CA` の PHP 8.5 deprecation 1 件は `config/database.php` 由来で本テストと無関係)。再生成自体は新 Task 構造で完結(AC 3 件)。
...
## B-A-01 題材を全面差し替え再生成: 面談二重予約(一意制約)へ(2026-05-31)

ユーザー討議の結論(並行制御の概念を残しつつ画面/採点で担保したい)に基づき、B-A-01 を旧「面談回数の二重消費(quota / `lockForUpdate`)」から「**面談予約で同一コーチ・同一時刻枠が二重予約される(DB 一意制約 `(coach_id, scheduled_at)` の欠如)**」へ全面差し替え。

- **2 層防御の確認**: 層1 = `Meeting\StoreAction::findAvailableCoaches` の `whereDoesntHave('meetingsAsCoach', status IN[reserved,completed])`(予約済コーチ除外、逐次/先行コミット済みのみ防ぐ)/ 層2 = `meetings.(coach_id, scheduled_at)` UNIQUE + INSERT の `UniqueConstraintViolationException`→`MeetingNoAvailableCoachException`(409)。
- **仕込み = 層2 のみ**(層1 温存): 「層1 のみ外す」は層2 が二重予約を物理的に防ぐため画面に症状が出ず(両 error path とも同一 `MeetingNoAvailableCoachException`)振る舞い採点不能 → 不採用。「層2 のみ外す」= 並行時のみ二重予約が顕在 = 純レース。
- **採点**: 画面クリック再現不可(並行が物理的に必要)→ ticket-spec §3.6 の **〔テスト〕タグ**で担保。決定論テストは同コーチ×同時刻の `canceled` Meeting 事前 INSERT(canceled は層1 をすり抜けるが UNIQUE は status 不問で効く)→ `StoreActionTest::test_unique_constraint_race_condition_converts_to_no_available_coach` 既存。
- **模範解答 PJ 懐疑的レビュー = ✅ 適切(コード変更なし)**: 一意制約 + catch + 決定論テストはすべて既存で理想形。旧 B-A-01 の `ConsumeQuotaAction` の `lockForUpdate` は正しいコードとして温存(ターゲット外)。
- **工数 6h→5h**(修正が migration の制約 1 行追加 + 既存 catch 活用と軽い)。Bug Advance 件数・並行性系 1 は不変。
- 整合更新: チケット `Bug/B-A-01_mentoring-02.md` 全面再生成 / README(Bug Advance 表・難易度ランプ・完成済リスト) / 要件ブレインストーミング(チケット表・サブカテゴリ・工数表)。

...
通知基盤(DB + メール配信 + Web 一覧)を構築しても、外部フロントエンド / モバイル / 第三者システムから通知を取得・既読化する手段が存在しない。将来の認証付き JS フロントによる通知ポップオーバーを実装する前提として、まず素の JSON API を組み上げる必要がある。
...
- [ ] `POST /api/v1/notifications/{notification}/read` で対象通知の既読化日時が現在時刻で更新され、JSON で成功レスポンスが返る(既読済みの通知に再度呼び出しても既読化日時は上書きされない)

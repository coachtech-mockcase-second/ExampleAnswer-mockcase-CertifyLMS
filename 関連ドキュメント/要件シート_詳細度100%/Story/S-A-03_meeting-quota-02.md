...
- [ ] 公開中以外(下書き / アーカイブ)の面談パックを指定した購入は拒否されてフラッシュエラーが表示され、購入記録は作成されない
- [ ] 正当な署名付きの決済イベント(Webhook)は受信され 200 が返る。署名検証に失敗したイベントは 400 が返り、購入記録 / 残数は一切変更されない
- [ ] 決済完了イベントの受信時、対応する購入記録が成功状態に遷移(決済確定日時 + 決済識別子を記録)し、同一トランザクション内で購入回数分の残数加算記録が作成される。加算直後から受講生の残数集計(ダッシュボード / 予約画面の数値)に反映される
- [ ] 決済失敗 / 期限切れイベントの受信時、対応する保留状態の購入記録が失敗状態に遷移し、残数加算は行われない
- [ ] 同一の決済完了イベントが再送されても残数加算は 1 回のみで、再送後も画面の残数表示は増えない(残数会計が崩れない)
- [ ] 決済完了後に失敗 / 期限切れイベントが逆順で到着しても、成功した購入は取り消されず、画面の残数表示も減らない
- [ ] 対応する購入記録が存在しないイベント / 未対応のイベント種別を受信した場合は 200 を返してスキップする(Stripe 側の再送を止める)
- [ ] コーチ / 管理者 / 学習中以外の受講生(招待中 / 修了 / 退会)は購入の選択画面にアクセスできない

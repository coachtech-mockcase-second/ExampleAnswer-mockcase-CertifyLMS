...
本チケットでは、受講生が残数 0 になっても自力でその場で追加面談パックを購入でき、決済完了後すぐに予約画面で残数加算が反映される状態を達成する。運営は admin で面談パックを管理するだけで、価格 / 回数の運用を柔軟にコントロールできる。

学習継続率の維持と追加収益動線の構築に加え、外部決済連携(Stripe Checkout / Webhook 冪等性 / 署名検証)という実務で頻出する題材を扱う。
...
### 決済イベントの受信(Webhook、認証なし・署名検証必須)

> ここでの「イベント」は外部決済サービス(Stripe)がサーバ間で送ってくる Webhook イベントを指す(LMS のデータベース / メール通知とは別物)。

- 外部決済サービス(Stripe)から送られる決済イベント(Webhook)を受け取る公開窓口(署名検証で改ざんを検出)
- 決済完了イベントで購入記録を成功にし、購入回数分の残面談回数を加算
- 決済失敗 / 期限切れイベントで購入記録を失敗にする(残数加算なし)
- 同一の決済完了イベントが複数回届いても残数加算は 1 回のみ(冪等性)
- 対応する購入記録がないイベント / 未対応のイベント種別は受信成功として無視
...
- [ ] 【受講生】面談パックの選択・購入開始
  1. 受講生(学習中)が `/meeting-quota/checkout` で公開中の面談パック一覧(名前 / 面談回数 / 価格、並び順)を閲覧できる
  2. 面談パックを選択して購入を実行すると Stripe Checkout の外部決済画面に遷移する
- [ ] 【受講生】決済完了画面
  1. Stripe で決済を完了して LMS の決済完了画面に戻ると、購入完了の案内 + ダッシュボードへの導線が表示される(残数加算は決済確定後に反映される旨を含む)
- [ ] 【システム】Webhook の署名検証
  1. 正当な署名付きの決済イベント(Webhook)は受信され 200 が返る
  2. 署名検証に失敗したイベントは 400 が返り、購入記録 / 残数は一切変更されない
- [ ] 【システム】決済完了イベントの処理
  1. 決済完了イベントの受信時、対応する購入記録が成功状態に遷移(決済確定日時 + 決済識別子を記録)する
  2. 同一トランザクション内で購入回数分の残数加算記録が作成され、加算直後から受講生の残数集計(ダッシュボード / 予約画面の数値)に反映される
- [ ] 【システム】決済失敗 / 期限切れイベントの処理
  1. 決済失敗 / 期限切れイベントの受信時、対応する保留状態の購入記録が失敗状態に遷移し、残数加算は行われない
- [ ] 【システム】同一イベント再送時の残数会計
  1. 同一の決済完了イベントが再送されても残数加算は 1 回のみで、再送後も画面の残数表示は増えない(残数会計が崩れない)
- [ ] 【システム】イベント逆順到着時の残数会計
  1. 決済完了後に失敗 / 期限切れイベントが逆順で到着しても、成功した購入は取り消されず、画面の残数表示も減らない
- [ ] 【システム】未対応イベント種別のスキップ
  1. 未対応のイベント種別を受信した場合は 200 を返してスキップする(Stripe 側の再送を止める)
- [ ] 【コーチ・管理者・受講生】購入画面のアクセス制御
  1. コーチ / 管理者 / 学習中以外の受講生(招待中 / 修了 / 退会)は購入の選択画面にアクセスできない
...
- 固定の受講生とデモ受講生に、決済状態の異なる購入記録（完了 / 保留 / 失敗 など）を投入する（状態ごとの履歴表示・完了分のみ残数に反映されることを確認）

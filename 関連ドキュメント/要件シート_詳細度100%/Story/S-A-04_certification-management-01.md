...
- [ ] 発行条件(公開中の模試すべてに合格)を満たさない受講登録では発行ボタンが無効表示されて実行できず、発行済(修了済)の受講登録では発行ボタンは表示されず PDF ダウンロード導線に変わる(二重発行の操作経路がない)
- [ ] 修了状態の受講登録に対する発行が成功すると、修了証レコード(受講生 / 受講登録 / 資格 / PDF パス / 発行日時)が作成され、同じ処理単位でプライベート保管領域に PDF ファイルが保存される
- [ ] 生成された PDF に受講生氏名 / 資格名 / 発行日(西暦表記)/ 固定 3 文言(「修了証」「上記の者は、本資格の所定の課程を修了したことを証する」「Certify LMS」)が日本語で文字化けせず描画され、資格コード / 試験区分 / カテゴリ等の過剰情報は含まれない
- [ ] 受講生本人 / 担当資格のコーチ / 管理者が自分の権限範囲の修了証ダウンロードにアクセスすると修了証の PDF がダウンロードされ、他受講生の修了証・コーチ担当外資格の修了証はダウンロードできない
- [ ] ダウンロード成功時に、修了証 PDF がファイル名 certificate-{修了証 ID}.pdf の添付ファイルとしてダウンロードされる
- [ ] 修了証レコードは存在するが保管領域に PDF ファイルが見つからない場合、ダウンロードできない

...
本チケットでは、全ロールが共通の設定画面から自分の情報を更新できる状態にする。プロフィール編集 / パスワード変更 / アバター画像変更 をタブ切替で完結させ、コーチには固定面談 URL 編集フィールドを追加する。修了済(graduated)受講生も本画面にはアクセスでき(プラン機能はロックされても自分の情報は触れる)、全ロール共通の基本機能として本 LMS の最低限のユーザー体験を整える土台となる。本チケットが揃わないと、運営側は些細な情報更新依頼まで管理者操作で代行することになり運用コストが増大する。
...
- [ ] 【受講生・コーチ・管理者】設定画面の表示
  1. 認証済の受講生 / コーチ / 管理者(修了済受講生を含む)が `/settings/profile` を開け、プロフィール / パスワードの 2 タブが表示される
  2. 未認証ユーザーはログイン画面にリダイレクトされる
- [ ] 【受講生・コーチ・管理者】プロフィール編集
  1. 氏名・自己紹介を更新成功すると、プロフィールタブへリダイレクトし成功メッセージ「プロフィールを更新しました。」が表示される
  2. メール欄は読み取り専用で表示され画面から編集できない
- [ ] 【コーチ・受講生・管理者】固定面談 URL 欄の出し分け
  1. コーチが編集フォームを開くと固定面談 URL 入力欄が表示され更新できる(空入力で値がクリアされる)
  2. 受講生 / 管理者のプロフィール編集には固定面談 URL 入力欄が表示されない
- [ ] 【受講生・コーチ・管理者】プロフィール編集フォームのバリデーション
  1. 違反時に該当タブで日本語エラーが表示され入力値も復元される(氏名 = 必須・50 文字以内 / 自己紹介 = 1000 文字以内 / 固定面談 URL〈コーチ〉 = URL 形式・500 文字以内)
- [ ] 【受講生・コーチ・管理者】パスワード変更
  1. 現在のパスワードが正しく新パスワードが 8 文字以上 + 確認一致のとき更新され、パスワードタブへリダイレクトし成功メッセージが表示される
  2. 現在パスワード不一致 / 新パスワード 8 文字未満 / 確認不一致のときはパスワードタブでエラーが表示されパスワードは更新されない
- [ ] 【受講生・コーチ・管理者】アバター画像のアップロード

...
> **制約列に集約する情報**: 外部キーの ON DELETE 戦略 / SoftDelete 採否 / 連動削除の仕組み。会話削除時のメッセージ連動削除は `cascadeOnDelete` で担保。
...
> レイヤー別に **クラス名 + ファイルパス + 1 行責務** を集約。クラス列挙はこのセクションに 1 箇所のみ。Action / Service / Repository は Advance 範囲(模範解答 PJ で採用、Basic 受講生は Controller 内完結も可)。
...
**Action** (`app/UseCases/AiChat/`, `app/UseCases/AiChatMessage/`、※ 模範解答 PJ で採用、Basic 受講生は Controller 内完結も可)

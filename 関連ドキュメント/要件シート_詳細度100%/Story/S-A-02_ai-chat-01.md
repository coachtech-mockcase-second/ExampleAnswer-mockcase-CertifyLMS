...
- 固定 student × 4 会話(全般 / 教材 の 2 モード、末尾が `Pending`(生成途中)の会話と末尾が `Error`(生成失敗)の会話を各 1 件含む)
- デモ受講生 6 × 1 会話(2 モードを順番に分配)
- 動作確認用途: 最新会話への自動遷移 / 各モードの履歴表示 / エラー状態メッセージの表示を実機確認できる

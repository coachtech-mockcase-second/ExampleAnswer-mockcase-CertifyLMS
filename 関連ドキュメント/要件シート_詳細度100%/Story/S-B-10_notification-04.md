...
- リマインダー配信コマンドを配信タイミング(前日 / 1 時間前)で切替起動
...
- [ ] リマインダー配信コマンドが前日リマインド(毎日 18:00)と 1 時間前リマインド(5 分間隔)の 2 種類で Schedule 登録されており、両方とも同時起動防止設定が付与されている
- [ ] 不正な配信タイミング指定(前日 / 1 時間前 以外)を渡した場合はエラーで弾かれ、エラーメッセージが標準エラーに表示される

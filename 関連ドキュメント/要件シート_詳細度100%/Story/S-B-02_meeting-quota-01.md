...
- [ ] 【管理者】面談パック一覧の絞り込み
  1. 面談パック管理画面で一覧を閲覧できる
  2. パック名キーワード(部分一致)+ ステータス(下書き / 公開中 / アーカイブ)フィルタで絞り込める
  3. 公開中を優先する並び順で表示される
- [ ] 【管理者】面談パックの新規作成
  1. 新規作成すると下書き状態で作成される
  2. 作成された面談パックの詳細画面へリダイレクトし、フラッシュメッセージ「面談パックを作成しました。」が表示される
- [ ] 【管理者】面談パックの編集
  1. 編集画面から基本情報(パック名 / 説明 / 面談回数 / 価格 / Stripe 価格 ID / 並び順)を更新できる
  2. このフォームではステータス(下書き / 公開中 / アーカイブ)は変更されない
  3. 成功時に詳細画面へリダイレクトし、フラッシュメッセージ「面談パックを更新しました。」が表示される
- [ ] 【管理者】面談パックの削除
  1. 下書き / アーカイブ状態の面談パックを削除すると一覧画面へリダイレクトし、フラッシュメッセージ「面談パックを削除しました。」が表示される
  2. 公開中は詳細画面に削除ボタンが表示されず削除できない
- [ ] 【管理者】面談パックのステータス遷移
  1. 公開(下書き → 公開中)/ アーカイブ(公開中 → アーカイブ)/ 下書きへ戻す(アーカイブ → 下書き)の 3 操作がそれぞれ遷移元の状態でのみ実行できる
  2. 成功時に詳細画面へリダイレクトし、フラッシュ表示(下書き→公開「面談パックを公開しました。」/ 公開中→アーカイブ「面談パックをアーカイブしました。」/ アーカイブ→下書き「面談パックを下書きへ戻しました。」)
- [ ] 【管理者】新規作成 / 編集フォームのバリデーション
  1. 違反時に日本語エラーが表示される(パック名 = 必須・100 文字以内 / 説明 = 任意・2000 文字以内 / 面談回数 = 必須・1〜100 の整数 / 価格 = 必須・0〜1,000,000 円の整数 / Stripe 価格 ID = 任意・255 文字以内 / 並び順 = 任意・0 以上の整数)
- [ ] 【受講生・コーチ】面談パック管理へのアクセス制御
  1. 受講生 / コーチは面談パック管理の画面(一覧 / 詳細 / 新規作成 / 編集)にアクセスできない

...
- [ ] コーチが面談設定タブの連携ボタンを押すと Google の認可画面に遷移し、認可完了後は面談設定タブに戻って「連携中」(連携日時付き) + フラッシュが表示される。受講生 / 管理者が連携ルートに直接アクセスすると 403
- [ ] コーチが解除ボタンを押すと連携情報が削除され、面談設定タブが「未連携」+ フラッシュ表示に戻る(Google 側のアクセス権も失効する)
- [ ] 認可コールバックでログイン中コーチと検証情報が一致しない、または永続トークン(リフレッシュトークン)が返らない場合は 400 が返り、連携情報は保存されず再連携を促すエラーが表示される
- [ ] 受講生の予約画面の空き枠で、連携済コーチは Google カレンダーで予定がある時刻が空き枠から外れ、連携なしコーチは Google を参照せず面談可能時間枠 + 既存予約のみで判定される
- [ ] 連携済コーチへの Google API 呼び出しが失敗してもエラー画面にならず、当該コーチは予定なし扱い(連携なしと同等)で空き枠が表示される
- [ ] 予約成立時の Google カレンダー連携:
  - 連携済コーチ: 面談 Event が作成され、面談に Event 識別子が保存される
  - 連携なしコーチ: Event 作成は走らず、識別子なしで予約が成立する
  - Event 作成 API が失敗しても予約自体(面談作成 + 面談回数消費 + 担当コーチ宛通知)は成立し、識別子なしになる
- [ ] 面談キャンセル時の Google カレンダー連携:
  - Event 識別子が紐づく面談: Google カレンダーの Event が連動削除される
  - 識別子がない / 削除 API が失敗した場合: キャンセル自体(キャンセル状態への遷移 + 面談回数返却)は成立する
- [ ] 連携済コーチのアクセストークンが期限切れの状態で空き枠取得 / Event 作成 / Event 削除が走ると、リフレッシュトークンで自動更新してリトライし、更新も失敗した場合は当該 API をスキップ(連携なし相当のフォールバック)して画面のフローは継続する

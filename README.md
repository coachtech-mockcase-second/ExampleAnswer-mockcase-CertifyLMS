...
| 評価ライン | 取得率（100 点満点換算）を COACHTECH 標準の成績基準 **S: 95 / A: 85 / B: 70 / C: 50** で判定（B 以上には Advance での得点が必須）|
...
全体配点に対する **取得率**（100 点満点換算）を、COACHTECH 標準の成績基準（**S: 95 点以上 / A: 85 点以上 / B: 70 点以上 / C: 50 点以上 / D: 50 点未満**）に当てて判定する。Basic 配点は全体の約 65% のため、**Basic のみでは C 評価が上限**。B 以上には Advance での得点が必須（AI 丸投げ耐性の中核）。
...
| 難易度 | 配点比（目安）| 設計意図 |
|---|---:|---|
| **Basic** | 約 65% | 教材範囲の完遂で C 評価まで到達。Basic のみでは B（70 点）に届かない |
| **Advance** | 約 35% | B 以上の全評価に Advance での得点が必須（S = 95 点には Advance ほぼ完答が必要）|

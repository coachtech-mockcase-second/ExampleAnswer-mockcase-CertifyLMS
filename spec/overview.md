...
| Git運用 | チケットごとにブランチを作成し、mainにPRを出す。コミットメッセージは何をしたかが明確にわかるように |
| PR記述 | 調査内容・原因分析（または設計判断）・実装内容・テスト確認の4項目を必須記載（詳細はチケット全体像セクションのAI対策を参照） |
...
受講生がAIツールでコードを生成しても「理解」が伴うよう、以下のガイドラインでチケットを設計する。

#### 第1層: チケット自体をAIだけでは解けない構造にする

| 手法 | 具体例 | なぜAIだけでは難しいか |
|------|--------|---------------------|
| **曖昧なバグ記述** | 「コース一覧にパフォーマンスの問題があります」（N+1とは書かない） | 受講生自身が問題を特定する必要がある |
| **既存パターン準拠** | 「お手本機能のテストパターンに倣ってテストを書け」 | 既存コードを読んで理解しないとパターンが分からない |
| **50%要件** | 要件が曖昧。コーチに質問して明確化する | AIは「何が分からないか」を質問できない |
| **規約準拠** | プロジェクト固有の規約・パターンに合わせる | AIはプロジェクト固有の文脈を自動で読み取れない |

#### 第2層: プロセスの可視化で「理解」を評価可能にする

各チケットのPRに以下の記述を必須とする（`docs/requirements-50.md` の開発ルールに記載）:
...
AIがコードを書いても、調査内容と原因分析は受講生自身の理解がないと書けない。

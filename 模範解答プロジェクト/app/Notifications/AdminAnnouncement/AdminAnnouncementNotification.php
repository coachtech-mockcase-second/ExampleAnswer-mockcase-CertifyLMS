...
namespace App\Notifications\AdminAnnouncement;
 ...
 *
 * 配信ターゲットは `AdminAnnouncement::target_type` で `AllStudents` / `Certification` / `User` を切替。
 * 通知行クリック後の遷移先は `/notifications` フルページ (お知らせ詳細は通知ペイロードに保持済)。
 */
final class AdminAnnouncementNotification extends BaseNotification
{
    public function __construct(public readonly AdminAnnouncement $announcement)
    {
        parent::__construct();
    }

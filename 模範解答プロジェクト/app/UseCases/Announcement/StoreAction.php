...
use App\Exceptions\Notification\AnnouncementInvalidTargetException;
use App\Exceptions\Notification\AnnouncementTargetNotFoundException;
 ...
 * 3. DB::transaction 内で Announcement INSERT + 対象受講生の解決 + dispatched_count / dispatched_at の確定
 * 4. commit 後に DB::afterCommit で各受講生へ通知発火 (Mail / Broadcast 副作用が rollback 漏れすることを防ぐ)
 *
 * @throws AnnouncementInvalidTargetException target_type と FK 列の組み合わせ不整合
 * @throws AnnouncementTargetNotFoundException 指定された資格 / ユーザーが存在しない
 */
final class StoreAction
{
    public function __construct(
        private readonly NotifyAnnouncementAction $notify,
    ) {}
    ...
    private function guardTargetConsistency(
        ...
        $valid = match ($type) {
            AnnouncementTargetType::AllStudents => $certificationId === null && $userId === null,
            AnnouncementTargetType::Certification => $certificationId !== null && $userId === null,
            AnnouncementTargetType::User => $userId !== null && $certificationId === null,
        };

        if (! $valid) {
            throw new AnnouncementInvalidTargetException;
        }

        if ($type === AnnouncementTargetType::Certification && ! Certification::where('id', $certificationId)->exists()) {
            throw new AnnouncementTargetNotFoundException;
        }
        if ($type === AnnouncementTargetType::User && ! User::where('id', $userId)->where('role', UserRole::Student)->exists()) {
            throw new AnnouncementTargetNotFoundException;
        }
    }

...
@section('content')
    ...
    <div class="mt-4">
        <h1 class="text-2xl font-bold text-ink-900">追加面談プランの新規作成</h1>
        <p class="text-sm text-ink-500 mt-1">下書きとして保存されます。公開は作成後に行います。</p>
    </div>

    <x-card class="mt-6" padding="lg" shadow="sm">
        <form method="POST" action="{{ route('admin.meeting-quota-plans.store') }}" class="space-y-5">
            @csrf
            ...
            <div class="flex items-center justify-end gap-2 pt-2">
                <x-link-button href="{{ route('admin.meeting-quota-plans.index') }}" variant="ghost">キャンセル</x-link-button>

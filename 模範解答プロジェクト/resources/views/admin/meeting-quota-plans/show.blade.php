...
@section('title', '追加面談プラン詳細 — ' . $plan->name)
...
@php
    ...
    $statusBadge = match ($plan->status) {
        MeetingQuotaPlanStatus::Published => ['variant' => 'success', 'label' => '公開中'],
        MeetingQuotaPlanStatus::Draft => ['variant' => 'warning', 'label' => '下書き'],
        MeetingQuotaPlanStatus::Archived => ['variant' => 'gray', 'label' => 'アーカイブ'],
    };
...
@section('content')
    <x-breadcrumb :items="[
        ...
        ['label' => '追加面談プラン管理', 'href' => route('admin.meeting-quota-plans.index')],
        ['label' => $plan->name],
    ]" />
    ...
    <div class="mt-4 flex items-start justify-between gap-4 flex-wrap">
        ...
        <div class="flex items-center gap-2">
            @can('update', $plan)
                <x-link-button href="{{ route('admin.meeting-quota-plans.edit', $plan) }}" variant="outline">
            ...
            @can('publish', $plan)
                @if ($plan->status === MeetingQuotaPlanStatus::Draft)
                    <form method="POST" action="{{ route('admin.meeting-quota-plans.publish', $plan) }}" class="inline-block">
                        @csrf
            ...
            @can('archive', $plan)
                @if ($plan->status === MeetingQuotaPlanStatus::Published)
                    <form method="POST" action="{{ route('admin.meeting-quota-plans.archive', $plan) }}" class="inline-block">
                        @csrf
            ...
            @can('unarchive', $plan)
                @if ($plan->status === MeetingQuotaPlanStatus::Archived)
                    <form method="POST" action="{{ route('admin.meeting-quota-plans.unarchive', $plan) }}" class="inline-block">
                        @csrf
                        <x-button type="submit" variant="secondary">
            ...
            @can('delete', $plan)
                @if ($plan->status !== MeetingQuotaPlanStatus::Published)
                    <form method="POST" action="{{ route('admin.meeting-quota-plans.destroy', $plan) }}" class="inline-block"
                          onsubmit="return confirm('この追加面談プランを削除しますか？(公開中は削除できません)');">
                        @csrf
                        @method('DELETE')

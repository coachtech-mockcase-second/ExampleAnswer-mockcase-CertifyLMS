...
@section('title', '追加面談プラン管理')

@php
    use App\Enums\MeetingQuotaPlanStatus;

    $statusBadge = fn (MeetingQuotaPlanStatus $s) => match ($s) {
        MeetingQuotaPlanStatus::Published => ['variant' => 'success', 'dot' => true],
        MeetingQuotaPlanStatus::Draft => ['variant' => 'warning', 'dot' => true],
        MeetingQuotaPlanStatus::Archived => ['variant' => 'gray', 'dot' => true],
    };
@endphp
...
@section('content')
    ...
    <div class="mt-4 flex items-center justify-between gap-4 flex-wrap">
        <div>
            <h1 class="text-2xl font-bold text-ink-900">追加面談プラン管理</h1>
            <p class="text-sm text-ink-500 mt-1">
                受講生が追加購入する面談回数 SKU の管理を行います。
                <span class="font-semibold text-ink-700">{{ $plans->total() }} 件</span>
            </p>
        </div>
        <x-link-button href="{{ route('admin.meeting-quota-plans.create') }}" variant="primary">
    ...
    <x-card class="mt-6" padding="sm" shadow="sm">
        <form method="GET" action="{{ route('admin.meeting-quota-plans.index') }}" class="grid gap-3 sm:grid-cols-[1fr_180px_auto]">
            <div class="relative">
            ...
            >
                ...
                @foreach (MeetingQuotaPlanStatus::cases() as $s)
            ...
            <div class="flex items-center gap-2">
                ...
                @if ($keyword || $status)
                    <x-link-button href="{{ route('admin.meeting-quota-plans.index') }}" variant="ghost">クリア</x-link-button>
                @endif
            </div>
        </form>
    </x-card>
        ...
        <div class="mt-6">
            <x-table>
                ...
                @foreach ($plans as $plan)
                    ...
                    <x-table.row>
                        <x-table.cell>
                            <a href="{{ route('admin.meeting-quota-plans.show', $plan) }}" class="block group">
                        ...
                        <x-table.cell class="text-right">
                            <x-link-button href="{{ route('admin.meeting-quota-plans.show', $plan) }}" variant="ghost" size="sm">

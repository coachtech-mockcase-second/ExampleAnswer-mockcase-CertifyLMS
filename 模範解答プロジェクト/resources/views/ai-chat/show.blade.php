...
@section('content')
    ...
    <div class="mt-4 grid grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)] bg-surface-raised border border-[var(--border-subtle)] rounded-3xl overflow-hidden shadow-sm h-[calc(100vh-180px)] min-h-[560px]">
        {{-- History sidebar --}}
        <aside class="hidden lg:flex flex-col bg-[#FCFEFD] border-r border-[var(--border-subtle)] overflow-y-auto">
        ...
        <div class="flex flex-col overflow-hidden bg-[#FCFEFD]">
            <header class="px-6 py-3 backdrop-blur-sm bg-white/70 border-b border-[var(--border-subtle)] flex items-center gap-3">
                ...
                <div class="ml-auto flex gap-1.5">
                    <form method="POST" action="{{ route('ai-chat.conversations.destroy', $conversation) }}"
                        ...
                        @method('DELETE')
                        <button type="submit"
                            class="w-8 h-8 rounded-[9px] border border-[var(--border-subtle)] text-ink-600 hover:border-danger-300 hover:text-danger-700 inline-flex items-center justify-center"
                            aria-label="この会話を削除">
            ...
            {{-- Composer --}}
            <div class="px-6 py-4 backdrop-blur-sm bg-white/70 border-t border-[var(--border-subtle)]">

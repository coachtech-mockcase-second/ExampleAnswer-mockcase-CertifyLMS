 ...
 *
 * 1. **3 モード網羅**: general(全般相談) / withEnrollment(資格相談) / withSection(教材相談) を混在し、
 *    AI チャット起動位置別の動線・履歴一覧フィルタが効くことを実機確認できるようにする。
 * 2. **固定 student に 3 モードすべて**: student@certify-lms.test には 3 会話を確実に作り、
 *    画面確認・PR スクショで安定参照できる「決まった会話」を用意する。
 * 3. **メッセージ status 網羅**: 1 会話だけ assistantPending メッセージで終わる状態にして、
 *    「処理中」表示の動作確認を可能にする。
...
final class AiChatSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if ($fixedStudent !== null) {
            $this->seedAllModesForFixedStudent($fixedStudent);
        }
    ...
    /**
     * 固定 student に 3 モード(general / withEnrollment / withSection)それぞれ 1 会話ずつ投入する。
     */
    private function seedAllModesForFixedStudent(User $student): void
    {
        $this->seedSingleConversation($student, mode: 0, daysAgo: 2);
        $this->seedSingleConversation($student, mode: 1, daysAgo: 5);
        $this->seedSingleConversation($student, mode: 2, daysAgo: 9, pendingTail: true);
    }

    /**
     * 指定モードで 1 会話を投入する。mode: 0 = general / 1 = withEnrollment / 2 = withSection。
     */
    private function seedSingleConversation(User $student, int $mode, int $daysAgo, bool $pendingTail = false): void
    {
        [$enrollment, $section] = $this->resolveContextForMode($student, $mode);

        $title = $this->titleForMode($mode, $enrollment);
        $script = $this->scriptForMode($mode, $enrollment, $section);
        ...
        $this->seedMessages($conversation, $script, $startedAt, $pendingTail);
    }

    /**
     * mode に応じて enrollment / section を解決する。
     *
     * mode=0 (general): 共に null
     * mode=1 (withEnrollment): student の最初の learning Enrollment を採用
     * mode=2 (withSection): Enrollment が属する Certification の Part → Chapter → Section を 1 件解決
     *
    ...
    private function titleForMode(int $mode, ?Enrollment $enrollment): string
    {
        return match ($mode) {
            0 => '学習計画の立て方を相談',
            1 => ($enrollment?->certification?->name ?? '資格') . 'の勉強方針について',
            2 => '教材の補足説明をリクエスト',
            default => 'AI 相談',
        };
    }
    ...
    /**
     * 会話シナリオ(user / assistant 交互、4-8 件)。
     *
     * @return list<array{role: AiChatMessageRole, content: string}>
     */
    private function scriptForMode(int $mode, ?Enrollment $enrollment, ?Section $section): array
    {
        $certName = $enrollment?->certification?->name ?? '資格';
        ...
        return match ($mode) {
            ...
            1 => [
                ['role' => AiChatMessageRole::User, 'content' => "{$certName}の出題傾向を踏まえた学習順序を教えてください。"],
                ['role' => AiChatMessageRole::Assistant, 'content' => "{$certName}は基礎理論 → 応用分野 → 過去問演習の 3 段階が定番です。\n基礎理論の段階で計算問題を毎日 5 問解く習慣をつけると後半が楽になります。"],
                ['role' => AiChatMessageRole::User, 'content' => '過去問は何年分くらい解けばよいですか?'],
                ['role' => AiChatMessageRole::Assistant, 'content' => "直近 5 年分を最低 2 周、可能なら 3 周が目安です。\n2 周目以降は誤答した問題に絞って復習すると効率的です。"],
                ['role' => AiChatMessageRole::User, 'content' => 'コーチへの相談も併用したほうがよいでしょうか?'],
                ['role' => AiChatMessageRole::Assistant, 'content' => "はい。AI 相談で勉強法の方針を立てた後、コーチ面談で個別の進捗を可視化してもらうのが効果的です。\nコーチには「○○ の章末問題で詰まっている」など具体的な状態を伝えると深い助言が得られます。"],
            ],
            2 => [
    ...
    /**
     * 会話に対し script の順序でメッセージを投入する。pendingTail=true の場合、最後の assistant メッセージを Pending 状態で残す。
     *
     * @param  list<array{role: AiChatMessageRole, content: string}>  $script
     */
    private function seedMessages(AiChatConversation $conversation, array $script, Carbon $startedAt, bool $pendingTail): void
    {
        foreach ($script as $i => $line) {
            $createdAt = $startedAt->copy()->addMinutes($i * 3);
            $isLastAssistantPending = $pendingTail
                && $i === count($script) - 1
            ...
            $message = $factory->state(['content' => $isLastAssistantPending ? '' : $line['content']])->create();

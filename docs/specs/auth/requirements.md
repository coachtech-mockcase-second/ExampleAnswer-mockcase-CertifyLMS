...
- **REQ-auth-011**: When 管理者が招待を発行する際（初回 / `$force=false`）, the system shall **`IssueInvitationAction::__invoke(string $email, UserRole $role, ?Plan $plan, User $invitedBy, bool $force = false)`**（v3 で `?Plan $plan` 引数追加、受講生招待では必須 / コーチ招待では NULL）を呼び、単一トランザクション内で (1) 同 email の `in_progress` / `graduated` User 不在を検査、(2) `users` 行を `status = invited` / `email` / `role` / `password = NULL` / `name = NULL` / **受講生招待時のみ `plan_id = $plan->id` / `plan_started_at = NULL`（オンボーディング時に確定）/ `plan_expires_at = NULL`（同上）/ `max_meetings = $plan->default_meeting_quota`**、**コーチ招待時は `plan_id = NULL` / `plan_started_at = NULL` / `plan_expires_at = NULL` / `max_meetings = 0`** で INSERT、(3) `invitations` 行を `user_id = users.id` / `status = pending` / `expires_at = now() + 7日`（`config('auth.invitation_expire_days', 7)` で env 設定可）で INSERT、(4) [[user-management]] の `UserStatusChangeService::record($user, UserStatus::Invited, $invitedBy, '新規招待')` を呼ぶ、(5) **受講生招待時のみ** [[plan-management]] の `UserPlanLog` を `event_type = assigned` で INSERT（呼出は [[plan-management]] の Service へ委譲）、(6) `InvitationMail` を dispatch する。

...
- [x] `app/Enums/AiChatMessageStatus.php` 作成（REQ-ai-chat-012）
  - `Pending` / `Completed` / `Error`、`label()` 含む
...
- [x] `config/ai-chat.php` 作成（REQ-ai-chat-050 / 080 / 081 / 090 / 100、design.md「Config」セクション参照）
  - `enabled` / `driver` / `gemini.*` / `daily_message_limit` / `history_window` / `title_generation_enabled` / `title_generation_prompt` / `system_prompt_template`
...
- [x] `sail artisan test --filter='AiChatPromptBuilderServiceTest'` 全 pass
- [x] `sail bin pint --dirty`
...
- [x] `app/UseCases/AiChatMessage/StoreAction.php` 作成（REQ-ai-chat-040 / 091）
  - constructor で `LlmRepositoryInterface $llm`、`AiChatPromptBuilderService $promptBuilder` を DI
  - `DB::transaction()` 内:
    ...
    6. catch LlmApiException:
       - assistant message UPDATE (error_detail / status=error)（user message は残す）
...
- [x] `tests/Feature/UseCases/AiChatMessage/StoreActionTest.php` 作成
  ...
  - Gemini エラー時: Fake が `LlmApiException` throw → assistant message が `error` 状態 + `error_detail` 保存（user message は残る）、`AiChatLlmFailedException` 再 throw
...
- [x] `app/Http/Requests/AiChatMessage/StoreRequest.php` 作成（REQ-ai-chat-040、`content` 1-2000 文字、authorize は conversation Policy::view）
...
- [x] `app/Http/Controllers/AiChatMessageController.php` 作成（REQ-ai-chat-040）
...
- [x] `tests/Feature/Http/AiChatMessage/StoreTest.php` 作成（REQ-ai-chat-040 / 060）
  ...
  - Gemini エラー時: Fake が例外 → 502 + assistant message が `error` 状態（user message は残る）
...
- [x] `resources/views/ai-chat/_partials/message-bubble.blade.php` 作成（REQ-ai-chat-012）
  ...
  - status=error: 赤背景 + アイコン（専用の再送信動線は持たず、受講生は入力フォームから同じ内容を送り直す）
  - status=pending: スピナー表示
...
- [x] `resources/js/ai-chat/chat-client.js` 作成（同期送信、fetch + JSON）（REQ-ai-chat-040）
...
- [x] `resources/js/ai-chat/message-renderer.js` 作成
  ...
  - error 状態のバブル表示（専用の再送信ボタンは持たず、受講生は入力フォームから送り直す）
...
> **🔵 完成チェックポイント**: ここまでで同期版 ai-chat の中核（会話 CRUD + メッセージ送受信）が完成。残る Step 10 は会話タイトル LLM 自動生成（任意拡張）。
...
- [x] `app/UseCases/AiChat/GenerateTitleAction.php` 作成（REQ-ai-chat-100）
  - constructor で `LlmRepositoryInterface $llm` を DI
  - `__invoke(AiChatConversation $conversation): ?string`
    ...
    6. 失敗時: `Log::channel('ai-chat')->warning(...)` + `null` 返却（呼出側で fallback 維持）
...
- [x] `tests/Feature/UseCases/AiChat/GenerateTitleActionTest.php` 作成（NFR-ai-chat-004）
  ...
  - Fake が exception throw → null 返却（呼出側で fallback 維持）
...
- [x] `sail artisan test --filter='GenerateTitle|StoreActionTitle'` 全 pass
- [x] `sail bin pint --dirty`
- [x] 動作確認（B-2 完成判定）:
  - 新規会話 + 初回メッセージ送信 → 応答直後にタイトルが「学習相談」等の意味のある短文に変わる
...
- [x] PR 説明（`tech.md`「PR 記述 7 セクション必須」準拠）:
  ...
  4. **実装内容**: 振る舞い単位で箇条書き（「受講生が AI と相談できるようになった」「教材から FAB で文脈付き相談できる」「同期応答で user / assistant メッセージを表示」等）
  5. **自動テスト**: 上記テスト結果サマリ
  6. **動作確認**: スクショ + 動画
  7. **レビュー観点 / 自己評価**: 不安な箇所（PHP-FPM worker 制約 / Gemini API クォータ管理 / etc.）

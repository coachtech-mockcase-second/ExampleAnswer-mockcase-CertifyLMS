...
- **NFR-meeting-quota-003**: The system shall 以下 INDEX を提供: `meeting_packs.(status, sort_order)` / `meeting_packs.deleted_at` / `meeting_quota_transactions.(user_id, occurred_at)` 複合 / `meeting_quota_transactions.related_meeting_id` / `meeting_quota_transactions.related_payment_id` / `payments.user_id` / `payments.(status, paid_at)` / `payments.stripe_payment_intent_id` UNIQUE / `payments.stripe_checkout_session_id` UNIQUE / `payments.deleted_at`。

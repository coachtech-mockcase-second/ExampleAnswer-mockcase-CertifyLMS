...
- **REQ-quiz-answering-001**: The system shall ULID 主キーを備えた `section_question_answers` テーブルを提供し、`user_id`（FK, restrictOnDelete）/ `section_question_id`（FK to `section_questions`, restrictOnDelete）/ `selected_option_id`（FK to `section_question_options`, nullOnDelete）/ `selected_option_body`（max 2000、スナップショット）/ `is_correct`（boolean, NOT NULL）/ `source`（enum `section_quiz` / `weak_drill`, NOT NULL）/ `answered_at`（datetime, NOT NULL）/ `created_at` / `updated_at` を保持する。
- **REQ-quiz-answering-002**: The system shall ULID 主キーを備えた `section_question_attempts` テーブルを提供し、`user_id`（FK, restrictOnDelete）/ `section_question_id`（FK, restrictOnDelete）/ `attempt_count`（unsigned int, default 0）/ `correct_count`（unsigned int, default 0）/ `last_is_correct`（boolean, default false）/ `last_answered_at`（datetime, NOT NULL）/ `created_at` / `updated_at` を保持する。`(user_id, section_question_id)` UNIQUE。
...
- **REQ-quiz-answering-008**: The system shall `SectionQuestionAnswer` を **非正規化保存**（user_id / section_question_id / source / answered_at / is_correct）で持ち、SectionQuestion / Section / Option が後から物理削除されても解答履歴の表示に支障が出ないようにする（`selected_option_body` の本文スナップショットで選択肢内容を保持）。

### 機能要件 — B. Section 紐づき問題演習エントリ

- **REQ-quiz-answering-020**: When 受講生が `GET /quiz/sections/{section}` にアクセスした際, the system shall `SectionQuizPolicy::view` で「親 Certification への active Enrollment（`status IN (learning, passed)`）が存在し、かつ Section / 親 Chapter / 親 Part がすべて `status=Published` である」ことを検証し、満たさなければ HTTP 404 を返す。
...
- **REQ-quiz-answering-023**: When 受講生が `GET /quiz/sections/{section}/questions/{question}` にアクセスした際, the system shall (1) `SectionQuizPolicy::view` 検証、(2) `$question->section_id === $section->id` を確認、(3) Question / Section / Chapter / Part が公開済であることを再検証、違反時は HTTP 404 を返す。
...
- **REQ-quiz-answering-053**: When C-052 を通過した際, the system shall 対象 `QuestionCategory` 配下の **公開済 SectionQuestion** を以下条件で抽出する: (a) `status = Published`、(b) `section.chapter.part` がすべて `Published`（cascade visibility）。**模試問題（`MockExamQuestion`）は出題対象に含めない**（v3 改修、`SectionQuestion` のみ）。並び順は `order ASC, id ASC`。
...
- **REQ-quiz-answering-081**: The system shall `SectionQuestionAnswerPolicy::create(User, SectionQuestion)` を以下で判定する: (a) `auth.id === $user.id`、(b) `$user->role === Student`、(c) `$user->status === UserStatus::InProgress`、(d) `$user` が `$question->section->part->certification_id` を `enrollments.status IN (learning, passed)` で受講中、(e) `$question->status = Published`、(f) 親 Section / Chapter / Part がすべて `Published`。違反は HTTP 403。
...
- **NFR-quiz-answering-003**: The system shall 以下 INDEX を提供: `section_question_answers.(user_id, answered_at)` / `(user_id, section_question_id)` / `(section_question_id, is_correct)` / `source` / `section_question_attempts.(user_id, section_question_id)` UNIQUE / `(user_id, last_answered_at)`。
...
- **解答取消・履歴削除** — 不変履歴
- **AI による問題生成 / 自動解説生成** — [[ai-chat]] が責務、本 Feature では扱わない
...
- **依存元**:
  ...
  - [[learning]] — 教材画面の「演習問題」タブから本 Feature の SectionQuestion 演習画面へ遷移、`SectionQuestionScoreService` を呼び出してスコア表示(挑戦回数 / 最高 / 最新)

## v3.5 改修 — SectionQuestionScoreService 公開 + 教材画面「演習問題」タブから到達 URL

### 機能要件 — SectionQuestionScoreService 公開

- **REQ-quiz-answering-180**: The system shall `App\Services\SectionQuestionScoreService` を提供し、`summarize(User $user, Section $section): SectionQuestionScoreSummary` メソッドで単一 Section の挑戦回数 / 最高スコア / 最新スコア / 正答率を集計して返す。
- **REQ-quiz-answering-181**: The system shall `SectionQuestionScoreService::batchSummarize(User $user, Enrollment $enrollment): Collection` メソッドで Enrollment 配下の全 Section を 1 ショット SQL で集計し、`Section.id => SectionQuestionScoreSummary` の Collection を返す(N+1 回避)。
- **REQ-quiz-answering-182**: The `SectionQuestionScoreSummary` shall readonly class として `attempt_count: int` / `best_score: int|null` / `latest_score: int|null` / `latest_answered_at: Carbon|null` / `accuracy_rate: float|null` を保持する。
- **REQ-quiz-answering-183**: The system shall [[learning]] の `Learning\ShowEnrollmentAction` から `SectionQuestionScoreService::batchSummarize` を呼び出すための公開契約とする(教材画面「演習問題」タブ表示時に Eager Load される)。

### 機能要件 — Section 詳細画面からの動線 (iField LMS `SectionQuizButton` 相当)

- **REQ-quiz-answering-185**: When 受講生が [[learning]] の Section 詳細画面 (`/learning/sections/{section}`) を表示した際, the system shall 読了ボタン付近に「Section 紐づき問題演習へ」リンクを表示する。リンクには当該 Section の最新スコアサマリ (`SectionQuestionScoreSummary`) を併記する。Section に SectionQuestion が 0 件の場合は本リンクを非表示にする。

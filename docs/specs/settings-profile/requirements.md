...
- **REQ-settings-profile-063**: When coach が `DELETE /settings/availability/{availability}` で枠を削除した際, the system shall `CoachAvailabilityPolicy::delete`([[mentoring]] 所有)で本人所有確認の上、物理削除する。
...
- **REQ-settings-profile-092**: When coach が「連携を解除」ボタンを押下した際, the system shall [[mentoring]] の `DestroyAction` を呼び出して Google 側トークンを revoke + DB 行を物理削除し、`/settings/profile?tab=meeting` へ redirect + Flash「Googleカレンダー連携を解除しました。」を表示する。

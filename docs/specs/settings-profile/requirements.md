...
- **REQ-settings-profile-090**: When coach が `GET /settings/profile` を表示した際, the system shall プロフィール画面下部に「Googleカレンダー連携」セクションを描画し、(a) 未連携時は「Googleカレンダーと連携する」ボタンを表示 (`mentoring.google-calendar.auth-url` ルートへ submit)、(b) 連携済時は「連携中: {gmail アドレス or `primary`}」+ 「連携を解除」ボタンを表示する。admin / student は本セクションを非表示にする。
- **REQ-settings-profile-091**: When coach が「Googleカレンダーと連携する」ボタンを押下した際, the system shall [[mentoring]] の `FetchAuthUrlAction` を経由して Google OAuth 認可 URL へ 302 redirect する。`state` には現プロフィール画面 URL を redirect 戻り先として埋め込む。
- **REQ-settings-profile-092**: When coach が「連携を解除」ボタンを押下した際, the system shall [[mentoring]] の `DestroyAction` を呼び出して Google 側トークンを revoke + DB 行を soft delete し、`/settings/profile` へ redirect + Flash「Googleカレンダー連携を解除しました。」を表示する。
- **REQ-settings-profile-093**: The system shall 本セクションの UI を `views/settings/profile/_partials/google-calendar.blade.php` に切り出して、`role === coach && CoachGoogleCredential` 有無で分岐する条件分岐を集約する。受講生 / 管理者の Blade 描画には影響しない。

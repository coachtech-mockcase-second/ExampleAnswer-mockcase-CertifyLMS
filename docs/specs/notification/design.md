...
**通知種別は受講生宛 + コーチ宛の 7 種類**(mock-exam 採点完了通知 + v3 撤回 3 種類を除いた値):
...
| 3 | `CompletionApprovedNotification` | 受講生本人 | **[[enrollment]] `ReceiveCertificateAction`**(受講生自己発火) |
...
| 7 | `AdminAnnouncementNotification` | 対象 student 集合 | 本 Feature の `AdminAnnouncement\StoreAction` |

> mock-exam 採点完了通知は撤回: 受講生は提出後の Controller redirect で結果画面に即時遷移するため通知不要。後で必要になれば本 Feature 側で dispatch 起点を追加。
...
| REQ-notification-080〜089 | `AdminAnnouncementController` + `StoreAction` |
...
- `NotifyQaReplyReceivedActionTest` / `NotifyAdminAnnouncementActionTest`

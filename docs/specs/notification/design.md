...
> **v3 改修反映**(2026-05-16):
> - **撤回された通知**: `CompletionApprovedNotification`(受講生の自己操作で完結、遷移先画面の PDF DL リンクで通知冗長) / `MeetingRequestedNotification` / `MeetingApprovedNotification` / `MeetingRejectedNotification`(mentoring 申請承認フロー撤回) / `PlanExpireSoonNotification`(MVP 外) / `StagnationReminderNotification`(滞留検知 v3 撤回)
...
**通知種別は受講生宛 + コーチ宛の 6 種類**(mock-exam 採点完了通知 + 修了証受領通知 + v3 撤回 3 種類を除いた値):
...
| 4 | **`MeetingReservedNotification`** | **担当コーチ宛のみ**(受講生宛は予約 UI で即時確認) | [[mentoring]] `Meeting\StoreAction` |
| 5 | `MeetingCanceledNotification` | 相手方(student キャンセル→coach / coach キャンセル→student) | [[mentoring]] `Meeting\CancelAction` |
| 6 | `MeetingReminderNotification` | 受講生 + コーチ両方 | 本 Feature の `SendMeetingRemindersCommand`(前日 18:00 + 1h 前) |
| 7 | `AnnouncementNotification` | 対象 student 集合 | 本 Feature の `Announcement\StoreAction` |

> mock-exam 採点完了通知 / 修了証受領通知は撤回: いずれも受講生本人の操作直後に Controller redirect で結果画面 / 修了証 DL リンクへ遷移するため、通知 dispatch は冗長。後で必要になれば本 Feature 側で起点を追加。
...
### 3. MeetingReserved 通知(v3 新規、コーチ宛のみ)
...
- **状態管理**: 素の JS モジュール (`notification-popover.js`) で `open` / `tab` / `items` / `loading` をクライアント側保持
- **タブ**: 全件 / 未読 (切替時に `GET /api/v1/notifications?tab=...&per_page=20` で内容 fetch、`credentials: 'include'` + Sanctum SPA Cookie 認証)
- **行構成**: 種別アイコン + タイトル + プレビュー + 経過時間 + 未読ドット
- **行クリック**: `POST /api/v1/notifications/{id}/read` (Sanctum Cookie 認証) → ポップオーバー close → `data.link_route` 遷移
...
**ポップオーバー内容取得エンドポイント** (`Api\V1\NotificationController::index`):

- リクエスト: `GET /api/v1/notifications?tab=all|unread&per_page=20`(`auth:sanctum` 必須、Sanctum SPA Cookie 認証)
- レスポンス: Laravel Resource 標準形式 (`data: [...]` + `meta: { current_page, last_page, per_page, total }`)
- 認可: `auth:sanctum` middleware で認証済ユーザーのみ通過、Controller 側で `$request->user()->notifications()` ベースに自分宛の通知のみ返却
...
sequenceDiagram
    ...
    participant API as Api/V1/NotificationController
    participant Pusher as Pusher Echo

    User->>Bell: クリック
    Bell->>Popover: ベルクリック (toggle)
    Popover->>API: GET /api/v1/notifications?tab=all (Sanctum Cookie)
    API-->>Popover: 最新 20 件 + meta 情報
    Note over Popover: ベル右下にフェードイン + 微スライド

    User->>Popover: 行クリック
    Popover->>API: POST /api/v1/notifications/{id}/read (Sanctum Cookie)
...
- `NotifyQaReplyReceivedActionTest` / `NotifyAnnouncementActionTest`
...
- `Notification/{Index,MarkAsRead,MarkAllAsRead,Panel}Test.php`
- `Admin/Announcement/{Index,Store,Show}Test.php`

...
> - **`UserStatusLog.event_type` カラムは持たない**（2026-05-17 設計修正、`from_status` / `to_status` で遷移を表現できるため冗長カラムを排除。`UserPlanLog.event_type` のみ 4 値で必要、引き算で揃える）
...
- **REQ-user-management-010**: The system shall 招待モーダルに `email`（必須 / email 形式）/ `role`（必須 / `coach` / `student` から選択。admin はシーダー作成のみで招待対象外）/ **`plan_id`（v3 新規、必須、[[plan-management]] の `Plan::published()` から `<x-form.select>` で選択）** の入力フィールドを提供する。
...
- **REQ-user-management-070**: The system shall `UserStatusLog` テーブルを提供する: ULID 主キー / `user_id` ULID FK / `from_status` `UserStatus` enum（**4 値**、v3）/ `to_status` `UserStatus` enum（**4 値**、v3）/ `changed_by_user_id` ULID FK nullable（システム自動変更時 NULL）/ `changed_reason` text nullable / `changed_at` datetime / timestamps。`(user_id, changed_at)` 複合 INDEX。ステータス遷移内容は `from_status` → `to_status` で表現するため、イベント分類用の追加カラムは持たない。
...
- **REQ-user-management-072**: The system shall `UserStatusChangeService::record(User $user, UserStatus $newStatus, ?User $changedBy, ?string $reason): UserStatusLog` シグネチャを提供する。本 Service は呼出時の `$user->status` を `from_status`、引数の `$newStatus` を `to_status` として `UserStatusLog` に INSERT する（`User.status` の UPDATE は呼出側責務、本 Service は INSERT only）。呼出側は record() を `User.status` が遷移前の状態で呼ぶ契約を守る必要がある。

...
完成形では Google Calendar OAuth 連携で空き枠取得 / event 作成 / Google Meet URL 自動生成を行い、`CoachAvailability` の繰り返し枠で空き時間を管理する。`CoachAvailability` モデル定義と空き枠取得は本 Feature が所有するが、**編集 UI**（`/settings/availability`）は [[settings-profile]] が所有する（責務分離、`product.md`「ロール共通画面の責務分担」準拠）。臨時シフト（COACHTECH の `EmployeeTemporaryShift` 相当）は **採用しない**。
...
- **REQ-mentoring-001**: The system shall ULID 主キー / `enrollment_id`（`enrollments.id` への外部キー）/ `coach_id`（`users.id` への外部キー、`role=coach`、**自動割当の結果として確定**）/ `student_id`（`users.id` への外部キー、`role=student`、`Enrollment.user_id` と冗長一致）/ `scheduled_at` datetime（開始時刻、終了時刻は常に `scheduled_at + 60 分`）/ `status` enum（**`reserved` / `canceled` / `completed`** の 3 値）/ `topic` text（受講生入力の話題）/ `canceled_by_user_id` ulid nullable（`users.id` への外部キー、`SET NULL`、`canceled` 遷移時のみセット）/ `canceled_at` datetime nullable / `meeting_url_snapshot` string nullable（`reserved` 遷移時に `coach.meeting_url` を焼き込み）/ `completed_at` datetime nullable / `meeting_quota_transaction_id` ulid（`reserved` 遷移時に [[meeting-quota]] の `consumed` トランザクションを参照、`canceled` 時の refund 元として利用）/ timestamps を備えた `meetings` テーブルを提供する。**`approved_at` / `rejected_reason` / `started_at` / `ended_at` カラムは持たない**（申請承認フロー撤回 + 入室手動操作撤回）。
...
- **REQ-mentoring-010**: The system shall ULID 主キー / `coach_id`（`users.id` への外部キー、`role=coach`）/ `day_of_week` tinyInteger（0=日曜, 6=土曜、Carbon 標準 `dayOfWeek`）/ `start_time` time / `end_time` time / `is_active` boolean default true / timestamps を備えた `coach_availabilities` テーブルを提供する。
...
- **REQ-mentoring-016**: The system shall ULID 主キー / `meeting_id`（`meetings.id` への外部キー、UNIQUE、cascadeOnDelete）/ `body` text / timestamps を備えた `meeting_memos` テーブルを提供する（1 Meeting : 1 MeetingMemo）。
...
### 機能要件 — Google Calendar OAuth 連携

> v3.6 追加 (2026-05-18): COACHTECH LMS 流の Google Calendar 連携を Advance スコープに正式追加。コーチが自分の Google アカウントを連携することで、(a) 受講生の予約画面で `CoachAvailability` の有効枠 ∩ コーチの GCal で予定なし のスロットだけが表示される、(b) 予約成立時にコーチの GCal にイベントが作成される、(c) キャンセル時にイベントも削除される。優先度概念や 30 分単位ビット演算は採用せず、`CoachMeetingLoadService` の「過去 30 日 completed 最少」と 60 分単位スロット運用を維持する。Google Meet URL の動的生成は採用せず、コーチの固定面談 URL (`users.meeting_url`) をイベント location / description に埋め込む。

- **REQ-mentoring-100**: The system shall ULID 主キー / `coach_id` (users.id への外部キー、UNIQUE) / `access_token` string(2048) / `refresh_token` string(512) / `calendar_id` string(255、デフォルト `primary`) / `connected_at` datetime / timestamps を備えた `coach_google_credentials` テーブルを提供する。1 コーチ : 1 認証情報を強制する (`coach_id` UNIQUE)。
- **REQ-mentoring-101**: The `CoachGoogleCredential` model shall `belongsTo(User::class, 'coach_id', 'coach')` リレーションを公開し、`User` model shall `hasOne(CoachGoogleCredential::class, 'coach_id')` リレーションを公開する。
- **REQ-mentoring-102**: The system shall `meetings` テーブルに `google_event_id` string(255) nullable カラムを追加する。GCal にイベント作成が成功した時のみ id を保存し、キャンセル時の event 削除リクエストに利用する。NULL の場合(GCal 連携なし、または event 作成失敗時)は GCal 削除をスキップする。
- **REQ-mentoring-103**: When コーチがプロフィール画面で「Googleカレンダーと連携」ボタンを押下した際, the system shall `App\UseCases\CoachGoogleCredential\FetchAuthUrlAction` を呼び、`scope=calendar.readonly+calendar.events` / `access_type=offline` / `prompt=consent` を指定した OAuth 認可 URL を生成して 302 redirect する。`state` パラメータには `coach_id` と redirect 戻り先パスを JSON でエンコードして含める。
- **REQ-mentoring-104**: When Google OAuth callback (`/settings/google-calendar/callback`) が `code` と `state` クエリで戻った際, the system shall `App\UseCases\CoachGoogleCredential\StoreAction` で `code` を `access_token` / `refresh_token` に交換し、`coach_google_credentials` に `coach_id = state.coach_id` で upsert する。完了後、`state.redirect_path` で示された画面へ redirect + Flash「Googleカレンダーと連携しました。」を表示する。
- **REQ-mentoring-105**: When コーチがプロフィール画面で「Googleカレンダー連携を解除」を実行した際, the system shall `App\UseCases\CoachGoogleCredential\DestroyAction` で Google 側のトークンを revoke 後、`coach_google_credentials` 行を物理削除する。連携解除後の `meetings.google_event_id` は保持する(過去予約の追跡用、将来再連携時に削除リクエストは送らない)。再連携時は `firstOrNew` で既存行を取得して `update` する。
- **REQ-mentoring-106**: If access_token が期限切れ (`401 Unauthorized`) の場合, then the system shall `refresh_token` を使って自動的にトークンを更新し、更新後のトークンで API リクエストを再試行する。`refresh_token` も無効化された場合は API 呼び出しをスキップし、`google_event_id` を NULL に更新する(ユーザーには「再連携が必要」を Flash 通知する選択肢を残す)。
- **REQ-mentoring-110**: When 受講生が `GET /enrollments/{enrollment}/meetings/availability?date=YYYY-MM-DD` で空き枠を取得する, the system shall 担当コーチ集合のうち `CoachGoogleCredential` が存在するコーチについては Google Calendar API の `freebusy.query` を `timeMin=date startOfDay` / `timeMax=date endOfDay` / `timeZone=Asia/Tokyo` で呼び出し、busy 時刻にぶつかるスロットを `available_coach_count` から除外する。`CoachGoogleCredential` 未連携のコーチは従来通り `CoachAvailability ∩ meetings` の判定のみ。
- **REQ-mentoring-111**: The system shall GCal API 呼び出しを `App\Services\Google\GoogleCalendarService::freebusy(CoachGoogleCredential, Carbon $start, Carbon $end): array` で集約し、戻り値を `[['start' => Carbon, 'end' => Carbon], ...]` の配列にする。GCal API エラー時は空配列を返してフォールバックする(busy 取得失敗で予約画面全体が壊れないようにする)。
- **REQ-mentoring-112**: When 受講生が `POST /enrollments/{enrollment}/meetings` で予約成立した直後、選出されたコーチが `CoachGoogleCredential` を持つ場合, the system shall `GoogleCalendarService::insertEvent(CoachGoogleCredential, Meeting): ?string` を呼び、event 作成成功時に戻り値 `event_id` を `meetings.google_event_id` に UPDATE する。Event の `summary` は `「{受講生名} と {資格名} の面談」`、`description` には受講生の `topic` + 「面談 URL: {coach.meeting_url}」、`location` には `coach.meeting_url` を入れる。event 作成失敗時は warning ログを残すが Meeting の reserve は成功扱い(GCal は付加機能)。
- **REQ-mentoring-113**: When 当事者が `POST /meetings/{meeting}/cancel` でキャンセルした直後、`meetings.google_event_id` が NULL でない場合, the system shall `GoogleCalendarService::deleteEvent(CoachGoogleCredential, string $eventId): void` を呼んで GCal の event を削除する。削除失敗 (410 Gone 等) は warning ログのみで Meeting のキャンセル自体は成功扱いとする。
- **REQ-mentoring-114**: The system shall GCal event の `summary` / `description` 文字列の組み立てを `lang/ja/mentoring.php` の `gcal.event_summary` / `gcal.event_description_template` に集約する(マジック文字列禁止、NFR-mentoring-006 と整合)。
...
## v3.6 改修 — Google Calendar 連携 (Advance 範囲)

`CoachAvailability` ベースの空き枠管理に加え、コーチが Google Calendar OAuth 連携を行うことで「コーチの個人 Google Calendar で予定が入っていない時刻」を更に絞り込む。空き枠の精度向上 + 予約成立時の GCal event 自動作成 + キャンセル時の event 自動削除がスコープ。詳細 REQ は上記 REQ-mentoring-100〜114 / NFR-mentoring-010〜013 を参照。

実装責務分担:

- **mentoring** が `coach_google_credentials` テーブル + `App\Services\Google\GoogleCalendarService` + `App\Services\Google\GoogleOAuthService` + `App\UseCases\CoachGoogleCredential\*` Actions + `CoachGoogleCredentialController` を所有する。
- **[[settings-profile]]** が `/settings/profile` 内に「Googleカレンダー連携」UI セクション(連携ボタン / 解除ボタン / 連携状態表示)を埋め込む(REQ-settings-profile 側で別途要件化)。
- **`MeetingAvailabilityService::slotsForCertification`** が GCal API を呼ぶ統合点。`Meeting\StoreAction` / `Meeting\CancelAction` が event 作成 / 削除の発火点。

Google Meet URL は **生成しない**。コーチがプロフィールに登録した固定面談 URL (`users.meeting_url`) を GCal event の `location` / `description` にそのまま入れ、`meetings.meeting_url_snapshot` にも引き続き焼き込む。
...
- **ビット演算による空き枠計算**: COACHTECH 流の 30 分刻みビット演算は採用せず、Eloquent range クエリで素直に算出（実用上の性能で十分、計算ロジックの可読性を優先）
...
- **承認時にコーチが個別 `meeting_url` を上書き入力**: 採用しない。Google Calendar OAuth で動的生成された Google Meet URL が `meeting_url_snapshot` に焼き込まれる（固定 URL を直接入力する経路は持たない）
...
- **NFR-mentoring-010**: The system shall Google API クライアントを `google/apiclient` パッケージ(`Google\Client` + `Google\Service\Calendar`)で実装する。OAuth credentials は `storage/google/credentials.json`(`.gitignore`)に配置するか、`.env` から `GOOGLE_OAUTH_CLIENT_ID` / `GOOGLE_OAUTH_CLIENT_SECRET` / `GOOGLE_OAUTH_REDIRECT_URL` を読む config を備える。
- **NFR-mentoring-011**: The system shall Google API 呼び出しを `Http::fake()` ではなく `Google\Client` の `setHttpClient(Guzzle)` を経由した自前 HTTP モッキングで testable にする。`MeetingAvailabilityService::slotsForCertification` のテストは GCal API を実呼び出しせず、`CoachGoogleCredential` の有無を切り替えて挙動を検証する。
- **NFR-mentoring-012**: The system shall GCal API の `freebusy.query` レスポンスをリクエスト 1 回あたり 1 コーチに限定し、複数コーチ分はループで個別に呼び出す(`items` に複数 calendar を渡す API は OAuth scope の都合上避ける)。N 名コーチ × 1 GCal API 呼び出し / 受講生予約画面表示 1 回、を上限とする。
- **NFR-mentoring-013**: The system shall `coach_google_credentials.access_token` / `refresh_token` の DB 保存をプレーンテキストで保持する(教材として OAuth フローの可視性を優先、本番運用時は `encrypt()`/`decrypt()` 又は Vault の導入を推奨と spec に明記)。
...
- **依存先**（本 Feature が前提とする）:
  ...
  - [[default-enrollment]]: `ResolveDefaultEnrollment` Middleware (予約画面のみ適用) + `<x-enrollment-switcher>` Component

## v3.5 改修 — 予約画面のみ [[default-enrollment]] 統合 + Switcher 埋込

### 機能要件 — 予約画面のみ Middleware 適用

- **REQ-mentoring-180**: When 受講生が予約画面 (`/meetings/create` or `/meetings/availability`) にアクセスした際, the system shall [[default-enrollment]] の `ResolveDefaultEnrollment` Middleware を適用し、default 資格の Enrollment コンテキストで予約画面を表示する。default NULL + Enrollment 2+ 件の場合は `<x-enrollment-switcher variant="empty-state" />` を含む Blade を返す(redirect しない)。
- **REQ-mentoring-181**: The system shall 履歴一覧 (`/meetings`) に `ResolveDefaultEnrollment` Middleware を **適用しない**。資格横断で複数面談を一覧表示する UX のため、Switcher で絞り込む UX は不適切。
- **REQ-mentoring-182**: The system shall 予約画面の default NULL + Enrollment 0 件の受講生に対し、教材ページレイアウトの main 領域に「資格カタログから申し込んでください」CTA を表示する。

### 機能要件 — 予約画面のみ Switcher 埋込

- **REQ-mentoring-185**: When 受講生が予約画面を表示した際, the system shall 画面上部に `<x-enrollment-switcher variant="inline" :current="$enrollment" />` Component を埋め込む。
- **REQ-mentoring-186**: The system shall 履歴一覧画面には inline Switcher を **埋め込まない**(資格横断画面のため不要)。

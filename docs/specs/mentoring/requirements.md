...
- **REQ-mentoring-100**: The system shall ULID 主キー / `coach_id` (users.id への外部キー、UNIQUE) / `access_token` string(2048) / `refresh_token` string(512) / `calendar_id` string(255、デフォルト `primary`) / `connected_at` datetime / timestamps / softDeletes を備えた `coach_google_credentials` テーブルを提供する。1 コーチ : 1 認証情報を強制する (`coach_id` UNIQUE)。
...
- **REQ-mentoring-105**: When コーチがプロフィール画面で「Googleカレンダー連携を解除」を実行した際, the system shall `App\UseCases\CoachGoogleCredential\DestroyAction` で Google 側のトークンを revoke 後、`coach_google_credentials` 行を soft delete する。連携解除後の `meetings.google_event_id` は保持する(過去予約の追跡用、将来再連携時に削除リクエストは送らない)。

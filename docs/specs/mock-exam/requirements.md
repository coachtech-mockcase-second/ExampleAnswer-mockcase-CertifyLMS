...
> **採点完了通知スコープ外**: 受講生は提出後の Controller redirect で結果画面に遷移するため、Notification 連携は本 Feature では持たない。Feature 横断で通知が必要になった場合は [[notification]] Feature 側で dispatch 起点を追加する。
>
...
- **REQ-mock-exam-001**: The system shall ULID 主キーを備えた `mock_exams` テーブルを提供し、`certification_id`(NOT NULL)/ `title`(max 100)/ `description`(NULL 可)/ `order`(NOT NULL, default 0)/ `passing_score`(NOT NULL, 1..100、百分率)/ `is_published`(NOT NULL, default false)/ `published_at`(NULL 可)/ `created_by_user_id` / `updated_by_user_id` / timestamps を保持する。**`time_limit_minutes` カラムは持たない**(E-3 撤回)。
- **REQ-mock-exam-002**: The system shall ULID 主キーを備えた **`mock_exam_questions` テーブル**(模試マスタの子リソースとして独立)を提供し、`mock_exam_id`(FK, NOT NULL, cascadeOnDelete)/ `category_id`(FK to `question_categories`, NOT NULL)/ `body`(text, NOT NULL)/ `explanation`(text, NULL 可)/ `order`(NOT NULL, default 0)/ timestamps を保持する。`difficulty` カラムは持たない。
- **REQ-mock-exam-003**: The system shall ULID 主キーを備えた **`mock_exam_question_options` テーブル**(新設)を提供し、`mock_exam_question_id`(FK, NOT NULL, cascadeOnDelete)/ `body`(text, NOT NULL)/ `is_correct`(boolean, NOT NULL)/ `order`(NOT NULL)/ timestamps を保持する。
- **REQ-mock-exam-004**: The system shall ULID 主キーを備えた `mock_exam_sessions` テーブルを提供し、`mock_exam_id`(FK, restrictOnDelete)/ `enrollment_id`(FK, restrictOnDelete)/ `user_id`(FK, 非正規化)/ `status` enum(`not_started`/`in_progress`/`submitted`/`graded`/`canceled`)/ `generated_question_ids`(JSON、MockExamQuestion.id 配列スナップショット)/ `total_questions` / `passing_score_snapshot` / `started_at` / `submitted_at` / `graded_at` / `canceled_at` / `total_correct` / `score_percentage` / `pass` カラムを保持する。**`time_limit_minutes_snapshot` / `time_limit_ends_at` カラムは持たない**(E-3 撤回)。
- **REQ-mock-exam-005**: The system shall ULID 主キーを備えた `mock_exam_answers` テーブルを提供し、`mock_exam_session_id`(FK, cascadeOnDelete)/ `mock_exam_question_id`(FK, restrictOnDelete)/ `selected_option_id`(FK to `mock_exam_question_options`, nullOnDelete)/ `selected_option_body`(max 2000、スナップショット)/ `is_correct`(boolean, NOT NULL, default false、採点時に確定)/ `answered_at`(datetime, NOT NULL)/ timestamps を保持する。`(mock_exam_session_id, mock_exam_question_id)` UNIQUE。
...
- **REQ-mock-exam-046**: When admin / 担当 coach が `DELETE /admin/mock-exams/{mockExam}` を呼んだ際, the system shall (1) `is_published=false` であること、(2) 当該 MockExam に紐づく `MockExamSession` がすべて `canceled` 状態であること、を `DestroyAction` で検証し、違反は `MockExamInUseException`(HTTP 409)、合格なら物理削除する(配下の `mock_exam_questions` / `mock_exam_question_options` は `cascadeOnDelete` で連動削除、`mock_exam_sessions.mock_exam_id` は `restrictOnDelete` で履歴保護されているため canceled セッションのみ残存している前提を満たす必要がある)。
...
- **REQ-mock-exam-064**: When admin / 担当 coach が `DELETE /admin/mock-exam-questions/{question}` で削除した際, the system shall (1) 親 MockExam が `is_published = false` であること、または (2) 削除後も MockExamQuestion が 1 件以上残ること、を検証し物理削除する(配下の `mock_exam_question_options` は `cascadeOnDelete` で連動削除、過去 `MockExamSession` の採点済 `mock_exam_answers.mock_exam_question_id` は `restrictOnDelete` で削除阻止されるため、採点済セッションが存在する MockExamQuestion は削除不可)。
...
- **REQ-mock-exam-223**: When MockExamAnswer.selected_option_id が NULL の場合(過去に選択した MockExamQuestionOption が物理削除済で `nullOnDelete` により NULL 化されたケース等), the system shall `is_correct = false` で確定する。採点済 MockExamAnswer が存在する MockExamQuestion の物理削除は `mock_exam_answers.mock_exam_question_id` の `restrictOnDelete` 制約で阻止されるため、採点に必要な MockExamQuestion / MockExamQuestionOption は必ず参照可能であることが保証される。
...
- 複数受講生での同時採点キュー化（採点は同期処理で完結、Queue 非同期化は採用しない）
- **採点完了通知 dispatch 全般** — 受講生は提出後の Controller redirect で結果画面に即時遷移するため、本 Feature では Notification 機構を持たない(`SubmitAction` に Notification 引数なし、Notification クラス・関連 migration なし)。通知が必要になれば [[notification]] Feature 側で dispatch 起点を追加
...
- **依存元**:
  ...
  - [[default-enrollment]] — `ResolveDefaultEnrollment` Middleware ([[learning]] / [[mock-exam]] / [[mentoring]] 予約画面に適用) + `<x-enrollment-switcher>` Component を消費

## v3.5 改修 — URL 再設計 + [[default-enrollment]] 統合 + Switcher 埋込

### 機能要件 — URL 再設計 (学習体験の包含構造)

- **REQ-mock-exam-180**: When 受講生が `/mock-exams` 直接アクセスした際, the system shall [[default-enrollment]] の `ResolveDefaultEnrollment` Middleware で default 資格の URL (`/learning/enrollments/{default}/mock-exams`) に自動 redirect する。default NULL + Enrollment 2+ 件の場合は教材ページレイアウト内に `<x-enrollment-switcher variant="empty-state" />` で「学習する資格を選択してください」UI を表示する(redirect しない)。
- **REQ-mock-exam-181**: The system shall 受講生向け模試一覧の URL を `/mock-exams` から `/learning/enrollments/{enrollment}/mock-exams` に再設計する。URL パスに Enrollment コンテキストを明示することで、`/learning` 配下の学習体験の包含構造として表現する。
- **REQ-mock-exam-182**: The system shall 受講生向け模試詳細 URL を `/learning/enrollments/{enrollment}/mock-exams/{mockExam}` に再設計する。MockExamSession 系の URL (`/mock-exam-sessions/{session}/*`) は維持(セッション ID で参照、enrollment コンテキスト不要)。

### 機能要件 — Switcher 埋込

- **REQ-mock-exam-185**: When 受講生が `/learning/enrollments/{enrollment}/mock-exams` 画面を表示した際, the system shall 画面上部に `<x-enrollment-switcher variant="inline" :current="$enrollment" />` Component を埋め込み、資格切替動線を提供する。
- **REQ-mock-exam-186**: The system shall サイドバーの「模試」リンクの遷移先を、auth user の `default_enrollment_id` から動的に解決された URL (`/learning/enrollments/{default}/mock-exams`) とする。`Route::has('mock-exam.catalog.index')` でガード。

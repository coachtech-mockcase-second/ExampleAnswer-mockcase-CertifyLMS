...
| **ファイル保存** | Laravel Storage（**private driver**: 修了証 PDF / **public driver**: 教材内画像・プロフィールアイコン、private 配信は `AttachmentController` 経由で Policy 当事者チェック + signed URL。chat 添付は E-2 で完全撤回）|
...
| 機能 | 実装手段 |
|---|---|
| 通常CRUD・教材閲覧・面談予約・チャット非同期 | **Blade テンプレート + フォーム送信 + Redirect**（PRG パターン、純 Laravel 標準）|
| 学習セッション auto-start / stop / 解答送信 / 結果画面 | **Blade + Form POST + Redirect**（JS 不使用、サーバ側 auto-start + 明示停止ボタン + Schedule Command の組み合わせ、2026-05-16 確定）|
| mock-exam の時間制限タイマー / マークシート式選択 / 提出 | **素のJS（`resources/js/mock-exam/`）+ fetch**（タイマーは JS が必須なドメイン要件のため例外採用）|
...
```markdown
## 概要
（チケット ID と、この PR で何ができるようになるか / 何が直るかを 1〜2 行。例: S-B-02 面談パックのマスタ管理 — 管理者が面談パックを作成・編集・公開できるようになる）

## 調査・設計判断
（何を読み・確認し、そこから何を判断したか。**AIには書けない、本人が読まないと書けない要素**。
　Story / Task: 参考にした既存実装 → この設計にした理由・代替案を退けた理由
　Bug: 調査の足取り → 特定した根本原因。**Why を中心に**、What の再掲は不要）
...
**PR 5セクションは AI 丸投げ排除設計の中核**（CLAUDE.md 参照）。受講生は **すべての PR で記述必須**。同内容の雛形を提供プロジェクトの `.github/PULL_REQUEST_TEMPLATE.md` として配布し、PR 作成時に自動展開させる。
...
|---|---|---|
| 概要 | — | レビュー起点、チケットとの紐づけ |
| 調査・設計判断 | ◎ | **本人が読まないと書けない**（既存コード参照）+ **Why の言語化**（AI は What を書きがち）|
| 実装内容 | ○ | 振る舞い単位の整理（コピペ排除）|
| 自動テスト | ○ | テスト存在の証明 |
| 動作確認（スクショ・動画） | ◎ | **AI 生成不可**（実機操作が必要）|

> 旧 7 セクション構成（関連チケット / 調査内容 / 原因分析・設計判断 / 実装内容 / 自動テスト / 動作確認 / レビュー観点・自己評価）は 2026-06-10 に 5 へ統合: 「関連チケット」は「概要」へ吸収、「調査内容」+「原因分析・設計判断」は執筆上一筆書きの内容（評価も同一採点行）のため一本化、「レビュー観点・自己評価」は形骸化リスクが高く廃止（不安箇所があれば任意で追記してよい）。

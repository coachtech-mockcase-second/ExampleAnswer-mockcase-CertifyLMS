...
| 13 | **notification** | student / coach | `Notification`（Laravel標準）+ `AdminAnnouncement` | Laravel Notification（**Database + Mail channel** 両方固定配信、ユーザー設定 UI なし）: ①**面談予約完了 → 担当コーチ宛のみ**（自動割当結果、受講生名 + 日時、受講生宛は予約 UI で即時確認のため通知不要）/ ②**修了証発行 → 受講生宛**（「修了証を受け取る」ボタン押下時の自己発火、Certificate PDF DL リンク含む）/ ③管理者お知らせ配信（admin → 全/特定資格/特定User への手動一斉 INSERT、`admin_announcements` テーブル）/ ④Q&A 自分質問への回答受信 → 受講生宛 / ⑤**新着 chat メッセージ**（コーチ → 受講生 + 受講生 → コーチ全員、Database + Mail）/ ⑥面談リマインド（前日 18 時 + 1 時間前、Schedule Command）。**mock-exam 採点完了通知 / 滞留検知通知 / プラン期限間近通知は採用しない**（採点完了は提出後 Controller redirect で結果画面に即時遷移、それ以外は MVP スコープ外）。受講生は通知一覧画面 `/notifications` で時系列閲覧 + 既読化、TopBar 通知ベル + サイドバーバッジで未読件数把握 | 既存実装+Basic拡張 | **Advance Broadcasting**（Pusher + WebSocket で TopBar 通知ベルへリアルタイム push、全通知種別を含む。chat メッセージもリアルタイム配信される）|

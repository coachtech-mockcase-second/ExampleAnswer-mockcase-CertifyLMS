...
| 13 | **notification** | student | `Notification`（Laravel標準）+ `AdminAnnouncement` | Laravel Notification（**Database + Mail channel** 全通知種別で両方固定配信、ユーザー設定 UI なし）: **受講生宛 7 種類のみ**（admin / coach 宛通知はゼロ、dashboard で集約確認に委ねる）: ①面談承認 / ②修了認定承認・修了証発行 / ③mock-exam 採点完了 / ④管理者お知らせ配信（admin → 全/特定資格/特定User への手動一斉 INSERT、`admin_announcements` テーブルで配信履歴管理、独立 Feature 化はせず本 Feature 内 Action として実装）/ ⑤Q&A 自分質問への回答受信 / ⑥**新着 chat メッセージ**（コーチ → 受講生のみ、Database + Mail）/ ⑦面談リマインド（前日 18 時のみ、Schedule Command）。受講生は通知一覧画面 `/notifications` で時系列閲覧 + 既読化、TopBar 通知ベル + サイドバーバッジで未読件数把握 | 既存実装+Basic拡張（教材外、繰り返し成功体験）| **Advance Broadcasting**（Pusher + WebSocket で TopBar 通知ベルへリアルタイム push、7 通知種別すべてを含む）|
...
| 16 | **settings-profile** | 全 | `User`（bio/avatar_url 拡張）, `CoachAvailability`（mentoringと共有）| **自分を管理する画面**（user-management = admin が他者を管理 と責務分離）。プロフィール表示・編集（名前 / メール / 自己紹介 / アイコン）+ パスワード変更 + **自己退会動線**（受講生・コーチ自身が `active → withdrawn` 遷移、`UserStatusLog` に記録、論理削除）+ **コーチのみ:面談可能時間枠設定**（CoachAvailability）。**通知設定 UI は提供しない**（[[notification]] は全通知 DB+Mail 両方を固定送信する設計、種別ごとの ON/OFF は採用しない） | 既存実装 | — |
...
| **admin / coach 宛通知**（修了申請受付 / 面談予約申請 / chat 新着 / 未回答 Q&A / 滞留検知 等のメール通知）| [[dashboard]] が「修了申請待ち / 未対応 chat / 未回答 Q&A / 今日の面談 / 滞留検知リスト」をロール別に集約表示するため、メール通知まで送るのは二重管理。admin / coach は dashboard で能動的に運用情報を確認するモデル |
| **面談予約申請 / 拒否 / キャンセル通知**（受講生・コーチ間の状態変化通知）| dashboard の面談一覧 + 通知一覧の代替で十分。能動的に確認可能なため、通知過剰を避ける |
| **学習途絶リマインド通知**（受講生本人へのメール）| `StagnationDetectionService` は dashboard の admin/coach 用滞留検知リスト表示で利用し、受講生本人へのリマインドメールは送らない（[[dashboard]] が受講生向けに「最終学習日」「ストリーク途切れ」を視覚化する責務） |

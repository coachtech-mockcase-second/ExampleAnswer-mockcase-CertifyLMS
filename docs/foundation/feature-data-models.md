...
| 3 | `CompletionApprovedNotification` | 受講生本人 | [[enrollment]] `ReceiveCertificateAction`（v3 自己発火） | DB+Mail（Mail に PDF DL URL） |
| 4 | `MeetingReservedNotification` | **担当コーチのみ** | [[mentoring]] `ReserveMeetingAction` | DB+Mail |
| 5 | `MeetingCanceledNotification` | 相手方（student↔coach） | [[mentoring]] `CancelMeetingAction` | DB+Mail |
| 6 | `MeetingReminderNotification` | 受講生 + コーチ両方 | `SendMeetingRemindersCommand`（前日 18:00 + 1h 前） | DB+Mail |
| 7 | `AnnouncementNotification` | 対象 student 集合 | `Admin\Announcement\StoreAction` | DB+Mail |

> mock-exam 採点完了通知 (`MockExamGradedNotification` / `NotifyMockExamGradedAction`) は撤回。提出後の Controller redirect で結果画面に即時遷移するため通知不要。
...
**設計判断**:
- **独自テーブルは `admin_announcements` のみ**: Notification 配信ログは Laravel 標準テーブルに任せ、`Announcement` だけ元情報用テーブルを足す
...
### 適切性評価
- ✅ **必要最低限**: 独自モデルは `Announcement` 1 個、他は Laravel 標準。8 通知種別の絞り込みも徹底（admin/coach 宛通知は chat 例外を除いて持たない）
- ✅ **最適化**: `DB::afterCommit` で発火、Mail は queue 化、Broadcasting は config 切替で Advance 領域、graduated/withdrawn 受信者は dispatch 前 skip
- ⚠️ **改善余地**: `Announcement` の `target_type` enum + `target_*_id` 列構成は **target が排他的に1つしか入らない**前提で動くが、DB 制約で表現されてない。CHECK 制約か Action 側のガードで明示したい
...
| `PlanInfoPanel` | `planName` + `courseDaysRemaining` + `meetingsRemaining` + `meetingPacks`（追加面談購入 CTA 用） |
...
**設計判断**:
- **`price` を持たない**: LMS 内決済は追加面談購入のみ（meeting-quota の `MeetingPack` が `price` を持つ）。初回プラン購入は LMS 外（公式サイト）で完結
...
**Feature 概要**: 面談回数の付与・消費・追加購入機能。`User.max_meetings`（初期付与）+ `MeetingPack` マスタ（SKU）+ `MeetingQuotaTransaction`（INSERT only 監査ログ）+ `Payment`（Stripe 決済）の 4 系統で構成。
...
| meeting_pack_id | FK restrict | 購入 SKU |
...
- **`payments` に `amount`/`quantity` スナップショット**: `meeting_packs` の `price`/`meeting_count` を将来変更しても、過去購入の金額・回数は不変
...
- ✅ **最適化**: `(user_id, occurred_at)` INDEX で残数集計と履歴一覧両方を高速化、`stripe_checkout_session_id` UNIQUE で Webhook 冪等性、Service の `history()` メソッドが Eager Loading 完備（`relatedMeeting.enrollment.certification`/`relatedPayment.meetingPack`/`grantedBy`）
...
| [[meeting-quota]] | `payments.amount` / `quantity` | 決済時の SKU 価格 / 数量 | MeetingPack の `price` 改定後も、過去の決済記録は当時の金額(払い戻し時の正確性) |
...
| 項目 | 該当 Feature | 推奨 |
|---|---|---|
| **CHECK 制約の不在** | notification（Announcement.target_*） / meeting-quota（MeetingQuotaTransaction.related_*） | MySQL 8 の CHECK 制約活用余地あり、ただし MySQL 互換性とのトレードオフ |

...
| 大項目 | 中項目 | 評価行の中身 |
|---|---|---|
| **① チケット完了** | **各チケット**（`{Feature略称}-{番号}: 概要` 形式、同 Feature を隣接配置）| 各チケットの受け入れ条件（振る舞い + 確認方法 + テスト実装 / 同梱テスト pass）を 1 AC = 1 採点行に転記。採点者は各 PR を開いて判定 |
| **② 横断評価** | **コード品質**（Pint / 命名 / Eloquent・FormRequest / N+1 新規混入なし / 未使用 use・不要コメントなし / **既存パターン踏襲**［Controller 薄い→Action/Service・テスト配置・namespace 規約］/ 型宣言・`declare(strict_types)`・readonly・PHPDoc・Collection 活用・**外部 API キーの .env 管理**［応用］）+ **テスト品質**（`sail artisan test` 全 pass / カバレッジ目標）+ **README**（外部 API の .env 等、新規に必要な設定の追記）+ **PR 5 セクション記述の全体達成度** | 全コードベース・全 PR 横断。配点を抑えた「明らかに不適切でないこと」の品質安全網 |
...
ただし、大項目 ② 横断評価 だけは振る舞いに依らない **最低限の品質安全網**（Pint / 命名 / N+1 / 既存パターンからの明らかな逸脱がないか）として配置する。「**明らかに不適切でないことを担保するチェックリスト**」レベルにとどめ、配点を抑えてここだけで A/S が決まらないバランスにする。

**横断（②）と PR・ドキュメントの扱い**:

- **横断は 1 大項目に統合**（旧「② 横断コード品質 / ③ 横断ドキュメント」を **② 横断評価** へ一本化）
...
| 4 | 評価シート | **2 大項目構造**（① チケット完了 / ② 横断評価［コード品質 + テスト品質 + README + PR 記述］）。配点 Basic 60〜70% / Advance 30〜40%、受け入れ条件レベルの振る舞いベースで評価 |
...
| coachtech-prepared-file/AssignedProject-mockcase-CertifyLMS | **受講生 Template 用**（GitHub Template Repository、2026-06-12 配置済み）| ✅ | `basic`（デフォルト）/ `advance`（**basic と同一内容**・フェーズ 2 の PR 先。Advance 向け画面も全ブランチに同梱され、`Route::has` 等の feature-aware ガードで実装までは非表示）|
...
- 自リポ内で各チケット = `feature/{ticket-id}` ブランチを切り、`basic`（または Advance フェーズでは `advance`）に向けて PR を提出。1 チケット = 1 PR、PR 5 セクション必須
...
Skills は **4個**（実装系 3 + Step 4 引き算系 1。Subagent は撤回、シンプル構成）。並列は git worktree + 別 Claude セッションで実現。
...
└── 関連ドキュメント/                       # 受講生・コーチ向け配布物
    ...
    ├── 評価シート.md                       # 2 大項目構造（チケット完了 / 横断評価）
    ├── 完全手順書_Basic.md / _Advance.md
    └── 復習教材/
```

**AssignedProject リポに配置されるもの**: `提供プロジェクト/` の中身 **のみ**（履歴リセットの Initial commit 1 つ、`basic` / `advance` 同一内容）。**`docs/` `.claude/` `関連ドキュメント/` は含めない**（構築側メタ情報。30% 版要件はスプシ + Google Docs で配布）。

...
| 6 | 配置 → AssignedProject リポへ（`docs/` `.claude/` `要件シート_詳細度100%/` は **含めない**、提供PJ + 30%要件等のみ）| 公開 |
...
BookShelf（150 時間）に対する **相対指標** として 250 時間を目標に置く。実装着手から PR 提出までの **総工数見積もり**（コードリーディング + 設計判断 + 実装 + テスト + 動作確認 + PR 記述）を指す。チケット粒度・件数は時間配分から逆算する（要件シート定義の最後で各チケットに工数仮置き → 合計が **250h ± 10%** に収まるかを点検）。
...
| 配点 | 値 | 設計意図 |
|---|---|---|
| **Basic 配点 = 60%** | Basic を満点取得 → 全体 60% 到達 = **A 評価ジャスト** | 教材範囲を真面目にこなした受講生に **A を保証**。Basic 100% でも S には届かない |
| **Advance 配点 = 40%** | S 評価（80%）到達には Advance から **20% 以上** 取得必須 | AI 丸投げで Advance を埋められないよう、**配点段階で AI 耐性を強制** |

> Basic 配点は **ジャスト 60% で固定**（上限 = 下限 = 60%）。Basic 満点で A 評価ジャスト到達、Advance を加えて 60〜80% の範囲を埋めて S を目指す設計。
> **S 取得必須 Advance チケット**（AI 耐性核 N 件の指定）の有無は **要件シート作成時に決定** する（チケット個別の AI 耐性スコアを見ながら判断）。
...
| # | 成果物 | 説明 |
|---|---|---|
| 1 | 提供プロジェクト | 受講生クローン用既存PJ。全Blade完成 / 実装済み機能（バグ込み）/ 未実装機能はBladeのみ + **`README.md`**（プロジェクト概要・セットアップ手順・使用技術・提出方法）+ **`ONBOARDING.md`**（既存コードリーディングガイド・主要ドメインモデル・既存パターン典型例の所在）。**完成形仕様（docs/）は含まない**（受講生は要件シート + 提供PJコード + ONBOARDING + コーチへのヒアリングで詳細化）|

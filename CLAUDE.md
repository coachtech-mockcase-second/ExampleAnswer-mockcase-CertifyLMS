...
評価方式: 全体配点に対する **取得率**（100 点満点換算）を COACHTECH 標準の成績基準 **S: 95 点以上 / A: 85 点以上 / B: 70 点以上 / C: 50 点以上 / D: 50 点未満** に当てて判定する（**2026-07 改定**: 旧「60% = A / 80% = S」は BookShelf 由来の案件固有想定で、採点 team 確認により標準基準へ全面改定。現配点構成〔Basic 65.2%〕では **Basic 完答 = C 評価** となり、B 以上に Advance の取得が必須）。
...
> **2026-07 改定注**: 上記の配点レンジ理論（下限 60% / 上限 80%）は旧評価ライン（A=60% / S=80%）前提の設計経緯として残す。標準基準（S=95 / A=85 / B=70 / C=50）の下では「Basic 完答 = A」は成立せず（Basic 65.2% = C 評価）、B 以上に Advance が必須。「Advance なしで S 不可」の担保は S=95% によってより強く成立する。評価ラインの実体は `関連ドキュメント/評価シート.md`「評価ライン」節（S=309 / A=277 / B=228 / C=163 点）。
...
| **`assigned-project-sync`** | `提供プロジェクト/` → 公開テンプレ AssignedProject（basic / advance）へのミラー同期（ls-files 方式 / 削除レビューゲート / 受講生視点の中立コミット文言 / リモート検証）。自己完結・直列 | なし |
| **`ticket-doc-sync <ID>`** | 30% チケット md 修正を配布物へ反映（軽微 = push〔同一 URL〕/ 実質 = supersede〔新 Doc + 旧版凍結〕を diff から判定・提案 → 実行 → マスタスプシのリンク差し替え → 検証）。自己完結・直列 | なし |

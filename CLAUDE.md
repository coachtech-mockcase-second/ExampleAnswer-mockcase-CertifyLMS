...
3 種別 × 計 42 件で構成（要件シート 100% で確定）。種別ごとのサブカテゴリと件数:

| 種別 | 件数 | サブカテゴリ別件数 |
|---|---:|---|
| **Story（機能開発）** | 15 | 新規機能の構築 9 / 既存機能の拡張 6 |
...
- **PR 7 セクション記述は全 PR を対象にするが、全 42 PR を逐一二値判定しない**。① の採点で各 PR を必ず開くので、その際に 7 セクション記述を **併せて観察** し、② で **全体の達成度を段階評価**（大半の PR で記述されていれば満点 / 軽微な欠落は小減点）。全 PR 精査の負担と「軽微な欠落で全体がバツ」になる過酷さを同時に避ける

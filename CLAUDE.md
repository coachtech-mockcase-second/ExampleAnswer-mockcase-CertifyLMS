...
| **② 横断評価** | **コード品質**（Pint / 命名 / Eloquent・FormRequest / N+1 新規混入なし / 未使用 use・不要コメントなし / **既存パターン踏襲**［Controller 薄い→Action/Service・テスト配置・namespace 規約］/ 型宣言・`declare(strict_types)`・readonly・PHPDoc・Collection 活用・**外部 API キーの .env 管理**［応用］）+ **テスト品質**（`sail artisan test` 全 pass / カバレッジ目標）+ **README**（外部 API の .env 等、新規に必要な設定の追記）+ **PR 7 セクション記述の全体達成度** | 全コードベース・全 PR 横断。配点を抑えた「明らかに不適切でないこと」の品質安全網 |
...
- **PR 7 セクション記述は全 PR を対象にするが、全 41 PR を逐一二値判定しない**。① の採点で各 PR を必ず開くので、その際に 7 セクション記述を **併せて観察** し、② で **全体の達成度を段階評価**（大半の PR で記述されていれば満点 / 軽微な欠落は小減点）。全 PR 精査の負担と「軽微な欠落で全体がバツ」になる過酷さを同時に避ける
...
| 要件シート100% のチケット追加・退役・統合（**件数変化**）| README（一覧・件数表・進捗トラッカー・工数合計）/ CLAUDE.md（件数サマリ）/ 評価シート（集計・大項目比率・Basic/Advance 比率・評価ライン **再計算**）/ 30%版（ファイル増減）|

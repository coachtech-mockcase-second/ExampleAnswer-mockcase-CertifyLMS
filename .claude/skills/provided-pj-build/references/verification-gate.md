...
1. 仕込んだ Bug / perf-Task の **代表テストを `path::method` で名指し実行** → **赤を確認**:
   ...
   ```
   緑のままなら危険信号。assert 欠落(silent gap)だけでなく、**テストの入力値がバグの誤った固定値と偶然一致**して PASS しているケースを疑う(例: フィルタが `draft` 固定になるバグを、テストが `draft` で絞り込み検証している)。テストを **バグの失敗が顕在化する入力**(固定値とは異なる値)に強化してから citation → reconcile.md「citation テストがバグの『失敗ケース』を踏まず偶然 PASS」。
...
> **500 の頻出原因 = 残す基盤ページが、削除したルートを無防備に `route('...')` 参照**(例: 履歴画面の「購入」ボタンが gut した checkout ルートを指す)。**gut した機能を消費する側のページ(ダッシュボード / 履歴 / 一覧等)を各ロールで実際に開いて描画**する(fetch でステータスだけ見ず、gut 影響ページは描画まで)。Story gut でルートを消したら by-ticket-type.md の「ルート削除前の Blade 全 grep」を先にやると、ここで 500 が出ない。なお gutted ルートへの直アクセスが **404**(ルート消滅)なのは正常、**500** は引き算の壊れ。

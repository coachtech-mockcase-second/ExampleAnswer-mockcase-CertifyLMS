...
**主参考: iField LMS（Kiro 流 SDD のベストプラクティス）**

- `/Users/yotaro/ifield-lms/.kiro/specs/contents-and-quizzes/` — 5ファイル構成（requirements.md / design.md / tasks.md / research.md / spec.json）。Certify は3ファイル（research/spec.json は任意）だが、各ファイルの構造・粒度・密度は強い参考になる
- 規模感: 大型 Feature で要件 10+、設計 200+行、tasks 50+ チェックボックス

**補助参考: COACHTECH LMS（ドキュメント粒度・タスク分割の参考）**

- `/Users/yotaro/lms/.claude/skills/steering/SKILL.md` — `1-requirements.md` / `2-design.md` / `3-tasklist.md` の段階構造とタスク粒度（1 タスク = 1 コミット）。Certify は番号プレフィクスなし命名（Kiro 流）だが、ドキュメント内容の粒度感は直接参考
...
3. `docs/specs/{name}/` 配下に3ファイルを Write で生成
4. 完了報告（生成行数 + 主要設計判断のサマリ）

...
3. **4 項目を初稿としてチャット上に提示**（コードブロックや表で見やすく）
4. **【重要】ユーザーフィードバックを待つ** — 認識ズレや追加要件があれば指摘される
5. **ブラッシュアップしたバージョンを再提示** — 修正箇所が明確に分かるように差分意識して書く
6. **ループ**: ユーザーが「OK で進めて」「Phase 1 に進んで」等と **明示的に同意するまで** ステップ 4-5 を繰り返す
...
| 5 | **Phase 0 実行**: 4 項目をチャット上に提示 → ユーザー壁打ち → 明示 OK までループ | 「## Phase 0」 |
| 6 | **Phase 1 開始**（ユーザー明示 OK 後のみ）: requirements → design → tasks の順に 3 ファイル生成 | 「## Phase 1」+ `templates/` |

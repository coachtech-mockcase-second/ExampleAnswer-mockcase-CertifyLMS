...
対象リポ・ブランチ構成が変わった場合は `CLAUDE.md`「リポジトリ・ブランチ」表を正とする(現行: `basic`=デフォルト / `advance`=同一内容)。
...
- `gh api repos/coachtech-prepared-file/AssignedProject-mockcase-CertifyLMS/branches --jq '.[] | "\(.name) \(.commit.sha[0:7])"'` で現在の両ブランチ SHA を取得。最終コミット日時・メッセージは `gh api "repos/.../commits?sha=basic&per_page=1" --jq '.[0].commit | "\(.committer.date) \(.message | split("\n")[0])"'` で確認し、どのくらい遅れているかを報告する
- **両ブランチの SHA が不一致なら想定外**(常に同一コミットを指す運用)。過去に片側だけ更新された可能性があるので、原因を確認しユーザーに相談してから進む
...
### 5. push(両ブランチ・force 禁止)

```bash
git push origin HEAD:basic HEAD:advance
```

- basic / advance は **常に同一コミット**を指す(内容差を作らない)
...
- 両ブランチ SHA 一致: `gh api .../branches --jq ...`
- **今回の変更の代表ファイル 2〜3 件**を `gh api "repos/.../contents/<path>?ref=basic" --jq '.content' | base64 -d | grep <期待パターン>` で実確認(例: 追加した Seeder のアカウント名、README のイメージ名)

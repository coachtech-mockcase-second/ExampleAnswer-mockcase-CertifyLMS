...
## ヘッダコメント標準形式（提供 PJ JS）

各モジュール先頭に「責務 / 監視する DOM フック（`data-*`）/ 公開する `init` 関数」を記したヘッダ DocBlock を付け、受講生が「このモジュールが何を見て何をするか」を一目で掴めるようにする。

```js
/**
 * {責務 1 行}
 * DOM フック: [data-xxx] で〜、[data-yyy] に〜。
 * 公開: initXxx()（app.js が DOMContentLoaded で呼ぶ）。
 */
```

- **API エンドポイントの URL / ルート名 / 送受信 JSON 設計 / サーバ側処理は書かない**（受講生が設計する BE のヒントになるため）。`data-*` フック（FE 内部契約）と挙動の説明に留める。
- 受講生の実装課題となる JS には、ヒントになるヘッダを付けない。
